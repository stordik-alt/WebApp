...
export function AppShell({ title, subtitle, actions, children }: { title: string; subtitle?: string; actions?: ReactNode; children: ReactNode }) {
  ...
  return <div data-app-title={title} data-read-only={isTester ? "true" : "false"} className="min-h-screen w-full min-w-0 overflow-x-clip bg-background text-foreground">
    ...
    <style>{`html:not(.dark) [data-app-title="Denní data"] .bg-slate-950\\/40,html:not(.dark) [data-app-title="Denní data"] .bg-slate-950\\/45,html:not(.dark) [data-app-title="Denní data"] .bg-slate-950\\/60,html:not(.dark) [data-app-title="Denní data"] .bg-slate-950\\/20,html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/35,html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/45,html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/55,html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/70{background-color:hsl(var(--card))!important;color:hsl(var(--foreground));}html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/35,html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/45,html:not(.dark) [data-app-title="Denní data"] .bg-slate-950\\/20{background-color:hsl(var(--background))!important;}html:not(.dark) [data-app-title="Denní data"] .bg-slate-900\\/55{background-color:hsl(var(--accent))!important;}html:not(.dark) [data-app-title="Denní data"] .text-cyan-200,html:not(.dark) [data-app-title="Denní data"] .text-amber-200{color:hsl(var(--foreground)/0.8)!important;}html:not(.dark) [data-app-title="Denní data"] .bg-rose-500.text-slate-950{background-color:hsl(var(--primary))!important;color:hsl(var(--primary-foreground))!important;box-shadow:0 8px 20px hsl(var(--primary)/0.16)!important;}html:not(.dark) .bg-cyan-400\\/10{background-color:hsl(190 60% 94%)!important;}html:not(.dark) .text-cyan-200{color:hsl(190 72% 25%)!important;}html:not(.dark) .text-red-200,html:not(.dark) .text-red-300{color:hsl(345 68% 34%)!important;}html:not(.dark) .bg-red-400\\/10{background-color:hsl(345 60% 95%)!important;}html:not(.dark) .border-red-400\\/40{border-color:hsl(345 55% 46%/0.55)!important;}html:not(.dark) .border-cyan-400\\/40{border-color:hsl(190 55% 42%/0.62)!important;}[data-read-only="true"] [data-testid="write-action"]{display:none!important;}`}</style>
    <aside className="fixed inset-y-0 left-0 z-40 hidden min-h-0 w-[270px] flex-col border-r border-sidebar-border/70 bg-sidebar/95 text-sidebar-foreground shadow-[20px_0_60px_-48px_black] backdrop-blur-xl lg:flex"><Brand /><NavList /><SidebarSettings /><div className="shrink-0 px-5 pb-5 text-[10px] leading-relaxed text-sidebar-foreground/35">Interní výrobní systém · cloudová data</div></aside>
    <div className="min-w-0 w-full overflow-x-clip lg:pl-[270px]"><header className="sticky top-0 z-30 w-full min-w-0 border-b border-border/70 bg-background/80 px-4 py-3 backdrop-blur-xl sm:px-6 sm:py-4 lg:px-8"><div className="mx-auto flex w-full max-w-[1600px] min-w-0 flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 flex-1 items-center gap-3"><Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="outline" size="icon" className="rounded-xl border-border/80 bg-card/70 lg:hidden" aria-label="Menu"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="left" className="flex h-dvh max-h-dvh min-h-0 w-[285px] max-w-[85vw] flex-col overflow-hidden border-sidebar-border bg-sidebar p-0 text-sidebar-foreground touch-pan-y"><Brand /><NavList onNavigate={() => setOpen(false)} /><SidebarSettings /></SheetContent></Sheet><div className="min-w-0"><div className="mb-0.5 hidden text-[10px] font-semibold uppercase tracking-[0.18em] text-primary/80 sm:block">Production monitoring</div><div className="flex items-center gap-2"><h1 className="truncate text-lg font-bold tracking-tight sm:text-xl">{title}</h1>{isTester?<span className="shrink-0 rounded-full border border-primary/30 bg-primary/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-primary">Tester · pouze čtení</span>:null}</div>{subtitle?<p className="mt-0.5 hidden truncate text-xs text-muted-foreground sm:block">{subtitle}</p>:null}</div></div>{!isTester&&actions?<div className="order-3 flex w-full min-w-0 flex-wrap items-center justify-center gap-2 sm:order-none sm:w-auto sm:justify-end">{actions}</div>:null}<div className="shrink-0"><span className="sm:hidden"><OptiShiftLogo compact /></span><span className="hidden sm:block"><OptiShiftLogo /></span></div></div></header><main className="mx-auto min-w-0 w-full max-w-[1600px] overflow-x-clip px-4 pb-24 pt-5 sm:px-6 sm:pt-6 md:pb-6 lg:px-8 lg:pb-8"><div key={pathname} className="animate-in fade-in duration-300"><RouteGuard>{children}</RouteGuard></div></main></div><BottomNav />
  </div>;
}
