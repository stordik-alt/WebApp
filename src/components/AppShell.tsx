...
function NavList({ onNavigate }: { onNavigate?: () => void }) {
  const { role } = useAuth();
  return <nav className="min-h-0 flex flex-1 flex-col gap-1 overflow-y-auto overscroll-contain touch-pan-y px-3 py-4 [scrollbar-width:thin]" style={{ WebkitOverflowScrolling: "touch" }}>{navFor(role).map(({ to, label, icon: Icon }) => <Link key={to} to={to} onClick={onNavigate} activeOptions={{ exact: to === "/" }} className="group relative flex min-h-10 shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-foreground/65 transition-[background-color,color,box-shadow,transform] duration-180 ease-out hover:-translate-y-px hover:bg-sidebar-accent/70 hover:text-sidebar-foreground" activeProps={{ className: "group relative flex min-h-10 shrink-0 items-center gap-3 rounded-xl bg-sidebar-primary/10 px-3 py-2.5 text-sm font-semibold text-sidebar-primary shadow-[inset_3px_0_0_hsl(var(--sidebar-primary))] hover:bg-sidebar-primary/15" }}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-sidebar-accent/45 transition-colors group-hover:bg-sidebar-accent"><Icon className="h-4 w-4" /></span><span className="truncate">{label}</span></Link>)}</nav>;
}
