  ...
  return (
    <>
      <Button
        type="button"
        variant="ghost"
        className="w-full justify-start gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-foreground/75 hover:bg-sidebar-accent hover:text-sidebar-foreground"
        onClick={() => setOpen(true)}
        aria-label="Nastavení vzhledu"
      >
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-sidebar-accent/45">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] w-[calc(100%-2rem)] max-w-3xl overflow-y-auto rounded-2xl border-border bg-card p-5 text-card-foreground shadow-2xl sm:p-6">
          <DialogHeader className="pr-8">
            ...
            <DialogDescription>
              Vyberte barevné schéma aplikace. Každé schéma má vlastní tmavou i světlou variantu.
            </DialogDescription>
          </DialogHeader>
            ...
            {SCHEMES.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group relative overflow-hidden rounded-2xl border p-3 text-left transition-all ${active ? "border-primary bg-primary/10 ring-2 ring-primary/20" : "border-border bg-background/60 hover:border-primary/50 hover:bg-accent/60"}`}
                  aria-pressed={active}
                >
                  ...
                  <div className="rounded-xl border border-border/70 bg-card p-2.5 shadow-sm">
                    <div className="mb-2 flex gap-1.5">
                      {item.colors.map((color) => <span key={color} className="h-5 flex-1 rounded-md border border-black/10" style={{ backgroundColor: color }} />)}
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <span className="h-8 rounded-md" style={{ backgroundColor: item.colors[0] }} />
                      <span className="h-8 rounded-md" style={{ backgroundColor: item.colors[1] }} />
                    </div>
                  </div>
                </button>
              );
