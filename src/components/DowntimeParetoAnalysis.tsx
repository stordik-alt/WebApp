  ...
  return (
    <Card className="p-4">
      ...
      {query.isLoading ? <p className="mt-4 text-sm text-muted-foreground">Načítám…</p> : query.isError ? <p className="mt-4 text-sm text-destructive">Data se nepodařilo načíst.</p> : rows.length === 0 ? <p className="mt-4 text-sm text-muted-foreground">V daném období nejsou žádné evidované odstávky.</p> : <>
        <div className="mt-4 h-[280px] w-full"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{ top: 8, right: 30, left: 0, bottom: 40 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" angle={-30} textAnchor="end" height={70} interval={0} tick={{ fontSize: 11 }} /><YAxis yAxisId="left" tickFormatter={(v) => `${v} min`} fontSize={11} stroke="hsl(var(--muted-foreground))" width={40} /><YAxis yAxisId="right" orientation="right" domain={[0, 100]} tickFormatter={(v) => `${v}%`} fontSize={11} stroke="hsl(var(--muted-foreground))" width={36} /><Tooltip formatter={((value: unknown, name: unknown) => { const n = typeof value === "number" ? value : Number(value); return name === "cumulativePct" ? [`${n.toFixed(1)} %`, "Kumulativně"] : [`${n.toFixed(0)} min`, "Minuty"]; }) as any} labelFormatter={((_: unknown, payload: any) => payload?.[0]?.payload?.fullName ?? "") as any} /><Bar yAxisId="left" dataKey="minutes" name="Minuty" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} /><Line yAxisId="right" dataKey="cumulativePct" name="Kumulativně" stroke="hsl(var(--chart-2))" strokeWidth={2} dot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>
