...
const reasonLabels: Record<string, string> = { MISSING_DATE: "Chybí datum", MISSING_SHIFT: "Chybí směna", MISSING_LINE: "Chybí linka", PRODUCT_NOT_FOUND: "Produkt není v databázi", PRODUCT_PROFILE_MISSING: "Chybí Product Profile", PRODUCT_PROFILE_INCOMPLETE: "Product Profile není kompletní", EMPLOYEE_UNMATCHED: "Zaměstnanec není přiřazen", POSITION_MISSING: "Chybí HA/TUP", POSITION_INVALID: "Neplatná pozice — povoleno je pouze HA nebo TUP", OEE_MISSING: "Chybí OEE", PERFORMANCE_MISSING: "Chybí výkon", AVAILABILITY_MISSING: "Chybí dostupnost", HOURLY_DATA_MISSING: "Chybí hodinová data", HOURLY_KPI_MISSING: "Hodinová data nemají platné KPI", DUPLICATE_RECORD: "Denní záznam již existuje" };
...
export function ImportApprovalQueue() {
  ...
  // Master Prompt sections 8-10: DUPLICATE_RECORD means a real employee/date/
  // shift/line/product match already exists in daily_records - fetch that
  // specific conflicting record (not just the blocker code) so the admin can
  // see and decide on the actual conflict instead of a generic label.
  const conflictRecordsQuery = useQuery({ queryKey: ["approval-conflict-records", selected?.id, (selected?.conflict_daily_record_ids ?? []).join(",")], enabled: Boolean(selected && (selected.conflict_daily_record_ids ?? []).length), queryFn: async () => { const { data, error } = await db.from("daily_records").select("id,employee_id,work_date,shift,line,product,oee,performance,available_time,screenshot_path,approval_status,updated_at").in("id", selected!.conflict_daily_record_ids ?? []); if (error) throw error; return data ?? []; } });
  const empNameFor = (id: string | null) => employees.find((e: any) => e.id === id)?.full_name ?? "?";
  // Master Prompt section 11 (Trace ID + step logging): the admin-facing
  // audit trail for a specific import - built on the existing
  // import_item_events table, which already captures every major lifecycle
  // transition with a timestamp.
  const eventsQuery = useQuery({ queryKey: ["approval-detail-events", selected?.id], enabled: Boolean(selected?.id), queryFn: async () => { const { data, error } = await db.from("import_item_events").select("id,event_type,from_status,to_status,payload,actor_id,created_at").eq("import_item_id", selected!.id).order("created_at"); if (error) throw error; return data ?? []; } });
  const eventTypeLabels: Record<string, string> = { VALIDATION_BLOCKED: "Validace zablokována", AUTO_APPROVED: "Automaticky schváleno", ADMIN_APPROVED: "Schváleno administrátorem", ADMIN_REJECTED: "Zamítnuto administrátorem", ERROR: "Chyba", RECOMPUTE_NEEDS_REVIEW: "Přepočet vyžaduje kontrolu" };
  ...
  // Master Prompt section 7 (Reimport): re-run the ORIGINAL screenshot
  // through the full pipeline from scratch, without deleting or overwriting
  // the record it replaces. Sequencing matters: reject the original FIRST
  // (guarded by .eq("status","PENDING_APPROVAL") + checking the returned row
  // so a double-click or a race with another admin action fails cleanly
  ...
  const reimport = useMutation({ mutationFn: async (item: PendingImport) => {
    ...
    await db.from("import_item_events").insert({ import_item_id: item.id, actor_id: session?.user?.id ?? null, event_type: "ADMIN_REJECTED", from_status: "PENDING_APPROVAL", to_status: "REJECTED", payload: { reason: "Reimport" } });

    const { data: blob, error: downloadError } = await supabase.storage.from("screenshots").download(item.screenshot_path);
    if (downloadError) throw downloadError;
    const file = new File([blob], item.screenshot_path.split("/").pop() || "reimport.png", { type: blob.type || "image/png" });
  ...
  // via p_confirm_conflict=true, which UPDATES the existing conflicting row
  // in place rather than inserting a second one - daily_records has a
  // UNIQUE(employee_id, work_date, shift, line, product_id) constraint, so a
  // true second row for the same natural key is never possible.
  const confirmConflict = useMutation({ mutationFn: async (item: PendingImport) => {
    const now = new Date().toISOString();
    const { data: rows, error } = await db.from("import_items").update({ status: "REJECTED", rejected_by: session?.user?.id ?? null, rejected_at: now, rejection_reason: "Potvrzený konflikt s existujícím záznamem.", conflict_resolution: "confirmed_conflict", conflict_resolved_by: session?.user?.id ?? null, conflict_resolved_at: now, completed_at: now }).eq("id", item.id).eq("status", "PENDING_APPROVAL").select("id");
    if (error) throw error;
    if (!rows?.length) throw new Error("Import byl mezitím změněn (např. jiným uživatelem).");
    await db.from("import_item_events").insert({ import_item_id: item.id, actor_id: session?.user?.id ?? null, event_type: "ADMIN_REJECTED", from_status: "PENDING_APPROVAL", to_status: "REJECTED", payload: { reason: "conflict_confirmed" } });
  }, onSuccess: (_d, item) => { clearDraft(approvalDraftKey(item.id)); setSelected(null); setPreviewUrl(null); qc.invalidateQueries({ queryKey: ["import-approval-queue"] }); toast.success("Konflikt potvrzen, import zamítnut. Původní záznam zůstal beze změny."); }, onError: (e: Error) => toast.error(e.message) });
  const confirmImportDespiteConflict = useMutation({ mutationFn: async (item: PendingImport) => {
    const { data, error } = await db.rpc("approve_import_item", { p_import_item_id: item.id, p_actor_id: session?.user?.id ?? null, p_confirm_conflict: true });
    if (error) throw error;
    if (data?.success === false) throw new Error(data?.message ?? "Schválení importu selhalo.");
    if (item.batch_id) await db.rpc("evaluate_batch_ha_tup_linkage", { p_batch_id: item.batch_id }).catch(() => {});
    return data;
  }, onSuccess: (data: any, item) => { clearDraft(approvalDraftKey(item.id)); setSelected(null); setPreviewUrl(null); qc.invalidateQueries(); toast.success(`Import schválen, konflikt vyřešen aktualizací existujícího záznamu. Aktualizováno: ${data?.updated_daily_records ?? 0}, nově vytvořeno: ${data?.created_daily_records ?? 0}.`); }, onError: (e: Error) => toast.error(`Import nebyl schválen: ${e.message}`) });
  ...
  return <div className="grid gap-4">
    <Card className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="font-semibold">Importy čekající na schválení</div><div className="text-sm text-muted-foreground">OCR je pouze návrh. Výkon, Dostupnost a OEE se vždy počítají ve 3. sekvenci a nelze je ručně měnit.</div></div><Badge variant={items.length ? "default" : "outline"}>{items.length} čeká</Badge></div></Card>
    {query.isLoading ? <Card className="p-6 text-sm text-muted-foreground">Načítám importy…</Card> : query.error ? <Card className="p-6 text-sm text-destructive">Nepodařilo se načíst importy: {(query.error as Error).message}</Card> : items.length === 0 ? <Card className="p-8 text-center text-sm text-muted-foreground">Žádný import nečeká na schválení.</Card> : <div className="grid gap-3">{items.map(item => <Card key={item.id} className="p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap gap-2"><Badge variant="secondary">PENDING</Badge>{item.trace_id && <Badge variant="outline" className="font-mono text-xs font-normal">{item.trace_id}</Badge>}{item.product_code && <Badge variant="outline">{item.product_code}</Badge>}{item.product_profile_status === "VALID" ? <Badge>Product Profile ✓</Badge> : <Badge variant="destructive">Profile {item.product_profile_status ?? "MISSING"}</Badge>}{item.ocr_confidence != null && <Badge variant="outline">OCR {Math.round(Number(item.ocr_confidence) * 100)}%</Badge>}</div><div className="mt-2 font-medium">{item.work_date ?? "Bez data"} · {item.shift ?? "Bez směny"} · {item.line ?? "Bez linky"}</div><div className="text-sm text-muted-foreground">{item.product_name || item.product_code || "Neurčený produkt"} · {rows.filter(r => r.import_item_id === item.id).length} zaměstnanců</div><div className="mt-2 flex flex-wrap gap-1">{(item.pending_reasons ?? []).map(r => <Badge key={r} variant="destructive" className="text-xs">{labelReason(r)}</Badge>)}</div></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void openPreview(item)}>Detail / screenshot</Button><Button variant="outline" disabled={reimport.isPending} onClick={() => reimport.mutate(item)}><RefreshCw className="mr-2 h-4 w-4" />Reimport</Button><Button variant="destructive" onClick={() => { setSelected(item); setRejectOpen(true); }}>Zamítnout</Button></div></div></Card>)}</div>}
    <Dialog open={!!selected && !rejectOpen} onOpenChange={v => { if (!v) { setSelected(null); setPreviewUrl(null); } }}><DialogContent className="max-h-[95vh] w-[calc(100%-1rem)] overflow-y-auto sm:w-[calc(100%-3rem)] sm:max-w-6xl">{selected && <><DialogHeader><DialogTitle className="flex flex-wrap items-center gap-2"><span>Kontrola importu · {selected.product_code ?? "bez produktu"}</span>{selected.trace_id && <Badge variant="outline" className="font-mono text-xs font-normal">{selected.trace_id}</Badge>}</DialogTitle></DialogHeader><MultiProductDetailSummary item={selected} /><div className="grid gap-5 lg:grid-cols-[1fr_1.2fr]"><div className="space-y-4"><Card className="overflow-hidden p-2">{previewUrl ? <a href={previewUrl} target="_blank" rel="noreferrer"><img src={previewUrl} alt="Originální screenshot importu" className="max-h-[520px] w-full rounded-lg object-contain" /></a> : <div className="p-8 text-center text-sm text-muted-foreground">Screenshot není dostupný.</div>}<div className="p-2 text-xs text-muted-foreground">Kliknutím otevřete originální screenshot ve větším zobrazení.</div></Card><Card className="p-4"><div className="font-semibold">Důvody kontroly</div><div className="mt-2 flex flex-wrap gap-2">{blockerList.length ? blockerList.map(x => <Badge key={x} variant="destructive">{labelReason(x)}</Badge>) : <Badge variant="outline">Žádný známý blocker</Badge>}</div></Card><Card className="p-4"><div className="font-semibold">Historie importu</div>{eventsQuery.isLoading ? <p className="mt-2 text-sm text-muted-foreground">Načítám historii…</p> : (eventsQuery.data ?? []).length === 0 ? <p className="mt-2 text-sm text-muted-foreground">Zatím žádné zaznamenané kroky.</p> : <ol className="mt-2 space-y-2 border-l pl-3">{(eventsQuery.data ?? []).map((ev: any) => <li key={ev.id} className="text-sm"><div className="flex flex-wrap items-center gap-2"><span className="font-medium">{eventTypeLabels[ev.event_type] ?? ev.event_type}</span>{ev.from_status && ev.to_status ? <span className="text-xs text-muted-foreground">{ev.from_status} → {ev.to_status}</span> : null}</div><div className="text-xs text-muted-foreground">{new Date(ev.created_at).toLocaleString("cs-CZ")}</div></li>)}</ol>}</Card>{blockerList.includes("DUPLICATE_RECORD") ? <Card className="border-amber-500/40 bg-amber-500/5 p-4"><div className="font-semibold text-amber-700 dark:text-amber-400">Konflikt s existujícím záznamem</div><div className="mt-1 text-sm text-muted-foreground">Pro stejného zaměstnance, datum, směnu, linku a produkt už existuje záznam. Zkontrolujte oba záznamy a rozhodněte, zda jde o skutečný konflikt.</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="rounded-lg border bg-background p-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Nový import</div><dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm"><dt className="text-muted-foreground">Datum</dt><dd>{selected.work_date ?? "–"}</dd><dt className="text-muted-foreground">Směna</dt><dd>{selected.shift ?? "–"}</dd><dt className="text-muted-foreground">Linka</dt><dd className="truncate">{selected.line ?? "–"}</dd><dt className="text-muted-foreground">Produkt</dt><dd className="break-all">{selected.product_name || selected.product_code || "–"}</dd></dl></div><div className="rounded-lg border bg-background p-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Existující záznam{(conflictRecordsQuery.data?.length ?? 0) > 1 ? "y" : ""}</div>{conflictRecordsQuery.isLoading ? <p className="mt-2 text-xs text-muted-foreground">Načítám…</p> : (conflictRecordsQuery.data ?? []).map((r: any) => <dl key={r.id} className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 border-t border-border/50 pt-2 text-sm first:border-0 first:pt-0"><dt className="text-muted-foreground">Zaměstnanec</dt><dd>{empNameFor(r.employee_id)}</dd><dt className="text-muted-foreground">Datum</dt><dd>{r.work_date}</dd><dt className="text-muted-foreground">Směna</dt><dd>{r.shift}</dd><dt className="text-muted-foreground">Linka</dt><dd className="truncate">{r.line}</dd><dt className="text-muted-foreground">Produkt</dt><dd className="break-all">{r.product ?? "–"}</dd><dt className="text-muted-foreground">OEE/Výkon/Dostup.</dt><dd>{r.oee ?? "–"}% / {r.performance ?? "–"}% / {r.available_time ?? "–"}%</dd><dt className="text-muted-foreground">Stav</dt><dd>{r.approval_status}</dd></dl>)}</div></div><div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" disabled={confirmConflict.isPending} onClick={() => confirmConflict.mutate(selected)}>Potvrdit konflikt (zamítnout tento import)</Button><Button variant="outline" disabled={confirmImportDespiteConflict.isPending} onClick={() => confirmImportDespiteConflict.mutate(selected)}>Potvrdit import (aktualizovat existující záznam)</Button></div></Card> : null}</div><div className="space-y-4"><Card className="p-4"><div className="font-semibold">OCR data – upravitelná</div>{recognizedProductCodes.length > 1 && <div className="mt-2 text-xs text-muted-foreground">Rozpoznané Product ID v hodinové tabulce: {recognizedProductCodes.map(code => <Badge key={code} variant="outline" className="ml-1">{code}</Badge>)}. Pole „Product ID" níže slouží pro hlavní/primární produkt záznamu – detail všech produktů je výše v „Rozpoznané produkty".</div>}<div className="mt-3 grid gap-3 sm:grid-cols-2">{([["work_date","Datum"],["shift","Směna"],["line","Linka"],["product_code","Product ID"],["product_name","Název produktu"],["norm_per_hour","Norma / h"]] as [string,string][]).map(([key,label]) => <label key={key} className="text-sm"><span className="mb-1 block text-xs text-muted-foreground">{label}</span><Input value={editing[key] ?? String((selected as any)[key] ?? "")} onChange={e => setEditing(p => ({ ...p, [key]: e.target.value }))} /></label>)}</div><Button className="mt-3" variant="outline" disabled={saveHeader.isPending} onClick={() => saveHeader.mutate()}>Uložit opravy a znovu validovat</Button></Card>
