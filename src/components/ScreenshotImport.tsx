...
export function ScreenshotImport({ employees, onImported }: { employees: Employee[]; onImported?: () => void }) {
  ...
  const createProductFamily = async () => {
    ...
    try {
      ...
      setProductFamily({ familyId, hProduct, tProduct }); setNewProductModalOpen(false); qc.invalidateQueries({ queryKey: ["products"] }); qc.invalidateQueries({ queryKey: ["product_norms"] }); toast.success(`Product ID „${name}" bylo vytvořeno a připraveno pro import.`); if (previewUrl) void runEmployeeStage(previewUrl);
  ...
  const continueAfterEmployees = async () => { if (!previewUrl) return; if (unresolvedEmployeeRows.length) { toast.warning("Nejprve přiřaďte nebo vytvořte všechny zaměstnance z OCR."); return; } setEmployeeSetupOpen(false); await runHourlyStage(previewUrl, rows); };
  ...
  const importBlocked = stage !== "ready" || (missingProductCodes.length > 0 && !productFamily);
