    ...
    try {
      ...
      if (!blockers.length && finalMatchedProduct && finalRows.length) {
        ...
        if (profile) {
          ...
          finalRows = refreshed.employeeRows; finalMatchedProduct = refreshed.matchedProduct; blockers = refreshed.blockers;
        } else {
          blockers = ["PRODUCT_PROFILE_MISSING"];
        }
      }

      if (!blockers.length) {
        const finalized = await finalizeImportItem(itemId, finalResult, finalRows, finalMatchedProduct, blockers, hourly, actualOee);
        updateItem(key, { status: finalized.status, blockers: [], result: finalResult, createdRecords: finalized.createdRecords });
      } else {
  ...
  return <>
    <div className="rounded-2xl border border-primary/30 bg-card/40 p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><div className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Import dat • 2.0 AUTO</div><h2 className="text-xl font-semibold">Hromadný import screenshotů</h2><p className="text-sm text-muted-foreground">Každý screenshot se zpracuje samostatně. Bezpečné záznamy jdou automaticky do denních dat, výjimky do Ke schválení.</p></div>
        <Button onClick={() => { reset(); setOpen(true); setTimeout(() => fileRef.current?.click(), 0); }} className="w-full sm:w-auto"><UploadCloud className="mr-2 h-4 w-4" />Vybrat screenshoty</Button>
      </div>
    </div>
      ...
      <DialogContent className="fixed inset-0 flex h-[100dvh] w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden rounded-none border-0 p-0 sm:inset-4 sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100vw-2rem)] sm:translate-x-0 sm:translate-y-0 sm:rounded-2xl sm:border">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 sm:px-6"><div className="mx-auto grid w-full max-w-6xl gap-4 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <section className="space-y-4">{selected && <>
            ...
            {previewUrl && <div className="overflow-hidden rounded-2xl border bg-black/20"><img src={previewUrl} alt={`Náhled ${selected.fileName}`} className="max-h-[45dvh] w-full object-contain" /></div>}
          </>}</section>
        </div></div>
