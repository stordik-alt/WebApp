...
const sideCode = (code: string, side: "HA" | "TUP") => { const raw = code.trim().replace(/\s+/g, ""); if (!raw) return ""; const upper = raw.toUpperCase(); const base = /^[HT]_/.test(upper) ? raw.slice(2) : raw; return `${side === "HA" ? "H" : "T"}_${base}`; };
...
export function ScreenshotImportV2({ employees, onImported }: { employees: Employee[]; onImported?: () => void }) {
  ...
  const runEmployees = async (imageDataUrl?: string) => {
    ...
    try {
      ...
      const next = (r.rows ?? []).map((row, i) => { const name = String(row.employee_name ?? "").trim(); const match = employeeMatch(name, activeEmployees); const lineText = String(result?.line ?? "").toLowerCase(); const productText = String(currentProduct ?? "").toUpperCase(); const position = /hand\s*assy|handassy/.test(lineText) || /^H_/.test(productText) ? "HA" as const : /touch\s*up|touchup|tup/.test(lineText) || /^T_/.test(productText) ? "TUP" as const : row.position === "TUP" ? "TUP" as const : "HA" as const; return { key: `${i}-${name}`, ocrName: name, employeeId: match?.id ?? null, position, oee: row.oee == null ? "" : String(row.oee), performance: row.performance == null ? "" : String(row.performance), availableTime: row.available_time == null ? "" : String(row.available_time), helpScore: "0", confidence: Number(row.confidence) || 0, include: true }; }).filter((r) => r.ocrName);
