  ...
  const saveImport = async () => {
    ...
    try {
      ...
      for (const row of selected) {
        ...
        records.push({ employee_id: row.employeeId, work_date: workDate, shift, line, product_id: product.id, position: row.position, oee: Number(row.oee) || 0, performance: Number(row.performance) || 0, available_time: Number(row.availableTime) || 0, help_score: 0, screenshot_path: screenshotPath, ...approval() });
      }
