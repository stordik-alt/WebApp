...
export async function persistOcrResult(itemId: string, result: OcrResult, products: Product[], employees: Employee[], profiles: ProductProfileContext[]) {
  ...
  const employeeRows = (result.rows ?? []).map((row, index) => {
    const wanted = normalizeName(row.employee_name); const exact = employees.find((e) => normalizeName(e.full_name) === wanted); const parts = wanted.split(" ").filter(Boolean); const matched = exact ?? employees.find((e) => parts.length > 1 && parts.every((p) => normalizeName(e.full_name).includes(p)));
...
export async function finalizeImportItem(itemId: string, result: OcrResult, employeeRows: any[], matchedProduct: Product | undefined, blockers: ImportBlocker[], hourly: OcrHourlyMetric[], actualOee: number | null) {
  ...
  const workDate = result.work_date!; const shift = result.shift!; const line = result.line!.trim(); const performance = average(hourly.map((m) => m.performance_pct)); const availability = average(hourly.map((m) => m.availability_pct));
  if (!hourly.length) throw new Error("Hourly data are required for AUTO processing.");
  ...
  const { data: itemMeta, error: metaError } = await db.from("import_items").select("screenshot_path,batch_id").eq("id", itemId).single(); if (metaError) throw metaError;
  let duplicateRows = 0; let createdRecords = 0;
  for (const row of employeeRows) {
    const { data: existing, error: existingError } = await db.from("daily_records").select("id").eq("employee_id", row.employee_id).eq("work_date", workDate).eq("shift", shift).eq("line", line).maybeSingle(); if (existingError) throw existingError;
    if (existing?.id) { duplicateRows += 1; await db.from("import_item_rows").update({ daily_record_id: existing.id, validation_status: "BLOCKED", admin_corrections: { duplicate: true } }).eq("id", row.id); continue; }
    const { data: record, error } = await db.from("daily_records").insert({ employee_id: row.employee_id, work_date: workDate, shift, line, product_id: matchedProduct.id, product: matchedProduct.code, position: row.position, oee: actualOee, performance: Number(performance.toFixed(2)), available_time: Number(availability.toFixed(2)), help_score: 0, screenshot_path: itemMeta.screenshot_path, approval_status: "approved", import_batch_id: itemMeta.batch_id }).select("id").single(); if (error) throw error;
...
export async function completeImportBatch(batchId: string) { const { data, error } = await db.from("import_items").select("status").eq("batch_id", batchId); if (error) throw error; const statuses = (data ?? []).map((x: any) => x.status); const hasError = statuses.some((s: string) => s === "ERROR"); const hasPending = statuses.some((s: string) => s === "PENDING_APPROVAL"); await db.from("import_batches").update({ status: hasError || hasPending ? "COMPLETED_WITH_ERRORS" : "COMPLETED", completed_at: new Date().toISOString() }).eq("id", batchId); }
