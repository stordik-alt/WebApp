...
function profileIsComplete(profile: ProductProfileContext | undefined) {
  ...
  return Boolean(
    profile.ha_subassy &&
      profile.tup_subassy &&
      Number(profile.h_norm_per_hour) > 0 &&
      Number(profile.h_capacity) >= 1 &&
      Number(profile.t_norm_per_hour) > 0 &&
      Number(profile.t_capacity) >= 1,
  );
}
...
export async function createImportItem(batchId: string, screenshotPath: string, sourceHash: string) {
  const { data, error } = await db
    .from("import_items")
    .insert({ batch_id: batchId, screenshot_path: screenshotPath, source_hash: sourceHash, status: "PROCESSING" })
    .select("*")
    .single();
  if (error) throw error;
  return data as { id: string };
...
export async function persistOcrResult(itemId: string, result: OcrResult, products: Product[], employees: Employee[], profiles: ProductProfileContext[]) {
  ...
  const employeeRows = (result.rows ?? []).map((row, index) => {
    ...
    return {
      ...
      raw_data: row,
      match_status: matched ? (exact ? "EXACT" : "MATCHED") : "UNMATCHED",
  ...
  const hourlyRows = (result.hourly_metrics ?? []).filter((m) => m.hour != null).map((m) => ({
    import_item_id: itemId,
    hour: Math.round(Number(m.hour)),
    product_code: m.product_code ?? null,
    role: inferRole(m.product_code),
    actual_output: m.actual_output,
    performance_pct: m.performance_pct,
  ...
  const { error: itemError } = await db.from("import_items").update({
    ...
    ocr_confidence: result.header_confidence ?? null,
    ocr_data: result,
    product_id: matchedProduct?.id ?? null,
    product_match_status: matchedProduct ? (normalize(matchedProduct.code) === normalize(productCode) ? "EXACT" : "MATCHED") : "NEW",
    product_profile_status: productProfileStatus,
  }).eq("id", itemId);
  if (itemError) throw itemError;

  const blockers = validate(result, employeeRows, matchedProduct, productProfileStatus, hourlyRows.length);
  await db.from("import_items").update({ pending_reasons: blockers }).eq("id", itemId);
  await db.from("import_item_events").insert({ import_item_id: itemId, event_type: "OCR_VALIDATED", from_status: "PROCESSING", to_status: blockers.length ? "PENDING_APPROVAL" : "AUTO_APPROVED", payload: { blockers } });
  return { blockers, employeeRows, hourlyRows, matchedProduct };
}

export async function finalizeImportItem(itemId: string, result: OcrResult, employeeRows: any[], matchedProduct: Product | undefined, blockers: ImportBlocker[], hourly: OcrHourlyMetric[]) {
  if (blockers.length) {
  ...
  const workDate = result.work_date!;
  ...
  const actualOee = Number(result.rows?.[0]?.oee);
  const safeOee = Number.isFinite(actualOee) ? actualOee : null;
  if (performance == null || availability == null || safeOee == null) throw new Error("Required KPI values are missing.");
  ...
  for (const row of employeeRows) {
    ...
    const { data: record, error } = await db.from("daily_records").insert({
      ...
      available_time: Number(availability.toFixed(2)),
      help_score: 0,
      screenshot_path: (await db.from("import_items").select("screenshot_path").eq("id", itemId).single()).data?.screenshot_path ?? null,
      approval_status: "approved",
      import_batch_id: (await db.from("import_items").select("batch_id").eq("id", itemId).single()).data?.batch_id ?? null,
    }).select("id").single();
    if (error) throw error;
  ...
  if (duplicateRows) {
    await db.from("import_items").update({ status: "PENDING_APPROVAL", pending_reasons: ["DUPLICATE_RECORD"], completed_at: null }).eq("id", itemId);
...
export async function completeImportBatch(batchId: string) {
  ...
  const status = hasError ? "COMPLETED_WITH_ERRORS" : hasPending ? "COMPLETED_WITH_ERRORS" : "COMPLETED";
  await db.from("import_batches").update({ status, completed_at: new Date().toISOString() }).eq("id", batchId);
}

function validate(result: OcrResult, rows: any[], product: Product | undefined, profileStatus: string, hourlyCount: number): ImportBlocker[] {
  ...
  if (!hourlyCount) blockers.push("HOURLY_DATA_MISSING");
...
function inferRole(code: string | null) {
  if (!code) return null;
  if (/^H_/i.test(code)) return "HA";
  if (/^T_/i.test(code)) return "TUP";
  return null;
}
