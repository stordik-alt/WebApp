...
export async function persistOcrResult(itemId: string, result: OcrResult, products: Product[], employees: Employee[], profiles: ProductProfileContext[]) {
  const productCode = result.product_code?.trim() || result.products?.[0]?.product_code?.trim() || null; const matchedProduct = productCode ? products.find((p) => normalize(p.code) === normalize(productCode)) : undefined; const productProfile = productCode ? profileForCode(profiles, productCode) : undefined; const productProfileStatus = !productProfile ? "MISSING" : profileIsComplete(productProfile) ? "VALID" : "INCOMPLETE";
  const employeeRows = (result.rows ?? []).map((row, index) => { const wanted = normalizeName(row.employee_name); const exact = employees.find((e) => normalizeName(e.full_name) === wanted); const parts = wanted.split(" ").filter(Boolean); const matched = exact ?? employees.find((e) => parts.length > 1 && parts.every((p) => normalizeName(e.full_name).includes(p))); return { import_item_id: itemId, row_index: index, ocr_employee_name: String(row.employee_name ?? "").trim() || null, employee_id: matched?.id ?? null, position: row.position ?? null, oee: row.oee, performance: row.performance, available_time: row.available_time, confidence: Number.isFinite(Number(row.confidence)) ? Number(row.confidence) : null, raw_data: row, match_status: matched ? (exact ? "EXACT" : "MATCHED") : "UNMATCHED", validation_status: matched ? "VALID" : "BLOCKED" }; });
  const { error: rowError } = await db.from("import_item_rows").insert(employeeRows); if (rowError) throw rowError;
  const hourlyRows = (result.hourly_metrics ?? []).filter((m) => m.hour != null).map((m) => ({ import_item_id: itemId, hour: Math.round(Number(m.hour)), product_code: m.product_code ?? null, role: inferRole(m.product_code), actual_output: m.actual_output, performance_pct: m.performance_pct, availability_pct: m.availability_pct, norm_per_hour: null, capacity: null, operator_count: null, actual_oee_pct: null, raw_data: m })); if (hourlyRows.length) { const { error } = await db.from("import_item_hourly").insert(hourlyRows); if (error) throw error; }
  ...
  const blockers = validateBase(result, employeeRows, matchedProduct, productProfileStatus); await db.from("import_items").update({ pending_reasons: blockers }).eq("id", itemId); return { blockers, employeeRows, hourlyRows, matchedProduct };
}
...
export async function finalizeImportItem(itemId: string, result: OcrResult, employeeRows: any[], matchedProduct: Product | undefined, blockers: ImportBlocker[], hourly: OcrHourlyMetric[], actualOee: number | null) {
  if (blockers.length) { await db.from("import_item_rows").update({ validation_status: "BLOCKED" }).eq("import_item_id", itemId).is("daily_record_id", null); await db.from("import_items").update({ status: "PENDING_APPROVAL", pending_reasons: blockers }).eq("id", itemId); await db.from("import_item_events").insert({ import_item_id: itemId, event_type: "VALIDATION_BLOCKED", to_status: "PENDING_APPROVAL", payload: { blockers } }); return { status: "PENDING_APPROVAL" as const, duplicateRows: 0, createdRecords: 0 }; }
  if (!matchedProduct) throw new Error("Cannot auto-process without a matched product.");
  const workDate = result.work_date!; const shift = result.shift!; const line = result.line!.trim(); const performance = average(hourly.map((m) => m.performance_pct)); const availability = average(hourly.map((m) => m.availability_pct)); if (!hourly.length) throw new Error("Hourly data are required for AUTO processing."); if (performance == null || availability == null || actualOee == null || !Number.isFinite(actualOee)) throw new Error("Required KPI values are missing.");
  const { data: itemMeta, error: metaError } = await db.from("import_items").select("screenshot_path,batch_id").eq("id", itemId).single(); if (metaError) throw metaError; let duplicateRows = 0; let createdRecords = 0;
  for (const row of employeeRows) { const { data: existing, error: existingError } = await db.from("daily_records").select("id").eq("employee_id", row.employee_id).eq("work_date", workDate).eq("shift", shift).eq("line", line).maybeSingle(); if (existingError) throw existingError; if (existing?.id) { duplicateRows += 1; await db.from("import_item_rows").update({ daily_record_id: existing.id, validation_status: "BLOCKED", admin_corrections: { duplicate: true } }).eq("id", row.id); continue; } const { data: record, error } = await db.from("daily_records").insert({ employee_id: row.employee_id, work_date: workDate, shift, line, product_id: matchedProduct.id, product: matchedProduct.code, position: row.position, oee: actualOee, performance: Number(performance.toFixed(2)), available_time: Number(availability.toFixed(2)), help_score: 0, screenshot_path: itemMeta.screenshot_path, approval_status: "approved", import_batch_id: itemMeta.batch_id }).select("id").single(); if (error) throw error; await db.from("import_item_rows").update({ daily_record_id: record.id, validation_status: "VALID" }).eq("id", row.id); createdRecords += 1; }
  if (duplicateRows) { await db.from("import_items").update({ status: "PENDING_APPROVAL", pending_reasons: ["DUPLICATE_RECORD"] }).eq("id", itemId); await db.from("import_item_events").insert({ import_item_id: itemId, event_type: "DUPLICATE_DETECTED", to_status: "PENDING_APPROVAL", payload: { duplicateRows, createdRecords } }); return { status: "PENDING_APPROVAL" as const, duplicateRows, createdRecords }; }
  await db.from("import_items").update({ status: "AUTO_APPROVED", completed_at: new Date().toISOString() }).eq("id", itemId); await db.from("import_item_events").insert({ import_item_id: itemId, event_type: "AUTO_APPROVED", to_status: "AUTO_APPROVED", payload: { createdRecords } }); return { status: "AUTO_APPROVED" as const, duplicateRows, createdRecords };
}

export async function approvePendingImport(itemId: string, actorId: string) {
  const { data: item, error: itemError } = await db.from("import_items").select("*").eq("id", itemId).eq("status", "PENDING_APPROVAL").single(); if (itemError) throw itemError;
  const { data: rows, error: rowsError } = await db.from("import_item_rows").select("*").eq("import_item_id", itemId).order("row_index"); if (rowsError) throw rowsError;
  const blockers: string[] = Array.isArray(item.pending_reasons) ? item.pending_reasons : [];
  if (!item.work_date || !item.shift || !item.line || !item.product_id || item.product_profile_status !== "VALID") throw new Error(blockers.length ? `Import nelze schválit: ${blockers.join(", ")}` : "Import nemá platný produkt, Product Profile nebo hlavičku.");
  if (!rows?.length || rows.some((r: any) => !r.employee_id || !r.position || r.oee == null || r.performance == null || r.available_time == null)) throw new Error("Před schválením musí být vyřešeni všichni zaměstnanci a všechny povinné hodnoty.");
  const { data: hourly, error: hourlyError } = await db.from("import_item_hourly").select("*").eq("import_item_id", itemId).order("hour"); if (hourlyError) throw hourlyError;
  const performance = average((hourly ?? []).map((m: any) => m.performance_pct)); const availability = average((hourly ?? []).map((m: any) => m.availability_pct)); const actualOee = average((hourly ?? []).map((m: any) => m.actual_oee_pct));
  if (!hourly?.length || performance == null || availability == null) throw new Error("Chybí platná hodinová data.");
  const oee = actualOee ?? Number(rows[0].oee); if (!Number.isFinite(oee)) throw new Error("Chybí platné OEE.");
  let createdRecords = 0;
  for (const row of rows) {
    const { data: existing, error } = await db.from("daily_records").select("id").eq("employee_id", row.employee_id).eq("work_date", item.work_date).eq("shift", item.shift).eq("line", item.line).maybeSingle(); if (error) throw error;
    if (existing?.id) throw new Error(`Denní záznam již existuje pro zaměstnance ${row.ocr_employee_name ?? row.employee_id}.`);
    const { data: record, error: insertError } = await db.from("daily_records").insert({ employee_id: row.employee_id, work_date: item.work_date, shift: item.shift, line: item.line, product_id: item.product_id, product: item.product_code, position: row.position, oee, performance: Number(performance.toFixed(2)), available_time: Number(availability.toFixed(2)), help_score: 0, screenshot_path: item.screenshot_path, approval_status: "approved", approved_by: actorId, approved_at: new Date().toISOString(), import_batch_id: item.batch_id }).select("id").single(); if (insertError) throw insertError;
    await db.from("import_item_rows").update({ daily_record_id: record.id, validation_status: "VALID" }).eq("id", row.id); createdRecords += 1;
  }
  await db.from("import_items").update({ status: "APPROVED", approved_by: actorId, approved_at: new Date().toISOString(), pending_reasons: [] }).eq("id", itemId);
  await db.from("import_item_events").insert({ import_item_id: itemId, actor_id: actorId, event_type: "APPROVED", from_status: "PENDING_APPROVAL", to_status: "APPROVED", payload: { createdRecords } });
