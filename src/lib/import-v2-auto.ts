...
export async function persistOcrResult(itemId: string, result: OcrResult, products: Product[], employees: Employee[], profiles: ProductProfileContext[]) {
  ...
  const thirdSequencePerformance = average((result.hourly_metrics ?? []).map((m) => m.performance_pct));
  const thirdSequenceAvailability = average((result.hourly_metrics ?? []).map((m) => m.availability_pct));
  const employeeRows = (result.rows ?? []).map((row, index) => {
    ...
    return { import_item_id: itemId, row_index: index, ocr_employee_name: String(row.employee_name ?? "").trim() || null, employee_id: matched?.id ?? null, position: row.position ?? null, oee: row.oee, performance: thirdSequencePerformance, available_time: thirdSequenceAvailability, confidence: Number.isFinite(Number(row.confidence)) ? Number(row.confidence) : null, raw_data: { ...row, performance: thirdSequencePerformance, available_time: thirdSequenceAvailability }, match_status: matched ? (exact ? "EXACT" : "MATCHED") : "UNMATCHED", validation_status: matched ? "VALID" : "BLOCKED" };
  });
