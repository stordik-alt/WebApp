...
function effectiveHourWeights(hourlyCount: number): number[] { if (hourlyCount <= 0) return []; const weights = Array.from({ length: hourlyCount }, () => 1); if (hourlyCount === 1) { weights[0] = (SHIFT_MINUTES - START_PREP_MINUTES - BREAK_MINUTES - END_CLEANUP_MINUTES) / 60; return weights; } weights[0] -= START_PREP_MINUTES / 60; weights[hourlyCount - 1] -= END_CLEANUP_MINUTES / 60; if (hourlyCount >= 3) weights[Math.floor(hourlyCount / 2)] -= BREAK_MINUTES / 60; return weights; }
...
function predictedShiftOutput(hourly: Record<string, unknown>[]): number | null { if (!hourly.length) return null; const weights = effectiveHourWeights(hourly.length); let total = 0; let used = 0; hourly.forEach((row, i) => { const norm = firstNum(row, ["norm_per_hour", "norm", "hourly_norm"]); if (norm === null || norm <= 0) return; const weight = weights[i] ?? 1; if (weight <= 0) return; total += norm * weight; used += weight; }); return used > 0 ? total : null; }
function normalizedPerformance(hourly: Record<string, unknown>[]): number | null { const expected = predictedShiftOutput(hourly); if (expected === null || expected <= 0) return null; const actual = hourly.map(actualOutputForHour).filter((v): v is number => v !== null).reduce((a, b) => a + b, 0); return actual / expected * 100; }
