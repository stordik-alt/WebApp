...
function relativeMinuteOfShift(shift: string | null, absoluteMinute: number): number {
  const start = SHIFT_START_MINUTES[shift ?? ""] ?? SHIFT_START_MINUTES[shiftForHour(Math.floor(absoluteMinute / 60))] ?? 0;
...
- Pokud pozice není čitelná, vrať position jako null. Aplikace ji následně může bezpečně odvodit z názvu linky (HandAssy => HA, TouchUp => TUP). Product ID nesmí samo o sobě určovat pozici pracovníka.
...
KRITICKÉ:
- Product ID je pouze kontext a nesmí ovlivnit rozpoznání pracovníků. Pokud pozice není čitelná, vrať position jako null; aplikace ji může odvodit z názvu linky.
...
function inferPositionFromLine(lineValue: unknown): "HA" | "TUP" | null { const line = text(lineValue).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[_-]+/g, " "); if (/hand\s*assy|handassy/.test(line)) return "HA"; if (/touch\s*up|touchup/.test(line)) return "TUP"; return null; }
function normalizeRows(parsed: Record<string, unknown>): OcrRow[] { const linePosition = inferPositionFromLine(firstText(parsed, ["line", "line_code", "line_name"])); const globalOee = firstNum(parsed, ["line_oee", "oee", "shift_oee", "oee_pct"]); return rawRowsFrom(parsed).map((row) => { const positionRaw = firstText(row, ["position", "operation", "role", "pozice"]); const normalizedRaw = positionRaw.trim().toUpperCase(); const position: "HA" | "TUP" | null = normalizedRaw === "HA" || normalizedRaw === "TUP" ? normalizedRaw : linePosition; const name = workerName(row); return { employee_name: name, position, oee: firstNum(row, ["oee", "oee_pct", "OEE"]) ?? globalOee, performance: null, available_time: null, confidence: firstNum(row, ["confidence", "certainty", "jistota"]) ?? (name ? 0.8 : 0.3) }; }).filter((r) => r.employee_name.length > 0); }
