...
export const extractHourlyWithContext = createServerFn({ method: "POST" })
  ...
  .validator((input: { imageDataUrl: string; context: HourlyStageContext }) => {
    ...
    if (!Array.isArray(input.context.profiles) && !Array.isArray(input.context.products)) throw new Error("3. sekvence potřebuje kontext z 1. sekvence.");
    return input;
  })
  .handler(async ({ data }): Promise<HourlyStageResult> => {
    const attempts: string[] = []; let parsed: Record<string, unknown> | null = null;
    for (const provider of providers()) { try { parsed = await callAi(provider, data.imageDataUrl, data.context); break; } catch (e) { const error = e as Error & { status?: number }; attempts.push(`${error.status ?? "error"}`); if (error.status !== 402 && error.status !== 429) break; } }
    ...
    const hourly_metrics: HourlyStageMetric[] = raw.map((row) => {
      ...
      const variant = findVariant(data.context, product_code, role); const norm_per_hour = variant?.norm ?? null; const capacity = variant?.capacity ?? null; const actual_output = actualOutput(row);
    ...
    return { hourly_metrics, predicted_shift_output, actual_shift_oee_pct, operator_count: data.context.operator_count, raw: JSON.stringify({ ...parsed, stage: 3, context: data.context, attempts }) };
  });
