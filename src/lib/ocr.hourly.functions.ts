...
export type HourlyStageMetric = {
  ...
  effective_norm?: number | null;
  ocr_norm_per_hour?: number | null;
  downtime_minutes?: number | null;
  downtime_before_production?: boolean | null;
};
...
function baseProductiveMinutesForHour(hour: number | null, screenshotTime: string | null): number {
  ...
  const shift = shiftForHour(h);
  const start = relativeMinuteOfShift(h, shift.start);
  let end = start + 60;
  const cutoff = screenshotRelativeMinute(screenshotTime, shift.start);
  if (cutoff != null) { if (cutoff <= start) return 0; if (cutoff < end) end = cutoff; }
  let minutes = Math.max(0, end - start);
  minutes -= Math.max(0, Math.min(end, shift.pauseEndRel) - Math.max(start, shift.pauseStartRel));
  minutes -= Math.max(0, Math.min(end, 7) - Math.max(start, 0));
  minutes -= Math.max(0, Math.min(end, 475) - Math.max(start, 475));
...
async function callAi(provider: AiProvider, imageDataUrl: string, context: HourlyStageContext, mode: "full" | "availability"): Promise<Record<string, unknown>> {
  ...
  const instruction = mode === "full"
    ? `Proveď 3. sekvenci OCR hodinové tabulky. Skutečně přečti KAŽDOU viditelnou řádku. Pro každou vrať hour, product_code, actual_output, norm_per_hour, performance_pct, availability_pct a downtime_minutes. Downtime_minutes čti výhradně ze sloupce Odstávka/Odstávky a vrať pouze konkrétní počet minut, pokud je v řádku skutečně uveden. Pokud je u odstávky pouze důvod nebo čas bez konkrétního počtu minut, vrať null. Vrať také downtime_before_production=true pouze tehdy, když je z tabulky zřejmé, že tato odstávka nastala před začátkem výroby v dané hodině; jinak false/null. Pokud jde o změnu výrobku na stejném pracovišti a odstávka je spojena s touto změnou, zachovej downtime_minutes. Dostupnost je SAMOSTATNÝ sloupec tabulky; nesmí být nahrazena OEE, výkonem ani null, pokud je číslo ve sloupci viditelné. Nejprve si vizuálně najdi hlavičku sloupce Dostupnost a potom čti hodnotu ve stejném sloupci pro každou řádku. Vrať také screenshot_time z hlavičky. Role pracoviště: ${context.role ?? "NEURČENA"}. Počet operátorů: ${context.operator_count}. Product Profile: ${contextLines || "žádný"}. Vrať pouze JSON.`
    : `Toto je DRUHÝ KONTROLNÍ PRŮCHOD. IGNORUJ výkon a OEE. Soustřeď se výhradně na samostatný sloupec DOSTUPNOST a sloupec Odstávka/Odstávky v hodinové tabulce. Najdi hlavičku Dostupnost a Odstávka/Odstávky. Pro každou viditelnou hodinu vrať přesně availability_pct a downtime_minutes. downtime_minutes vrať jen při konkrétním počtu minut; důvod nebo čas bez minut není číslo. Vrať také hour a product_code, aby šly hodnoty přiřadit ke správné řádce. Vrať downtime_before_production=true jen pokud je odstávka před začátkem výroby v dané hodině. Vrať pouze JSON.`;
  const system = mode === "full"
    ? `Jsi přesný OCR nástroj výrobní tabulky. Vrať pouze JSON {"screenshot_time":"HH:MM nebo null","hourly_metrics":[{"hour":číslo nebo null,"product_code":"kód nebo null","actual_output":číslo nebo null,"norm_per_hour":číslo nebo null,"performance_pct":číslo nebo null,"availability_pct":číslo nebo null,"downtime_minutes":číslo nebo null,"downtime_before_production":true|false|null}]}. Hodnoty procent vracej jako čísla bez %. Nikdy nezaměňuj sloupce. Norma je OCR norma přímo ze screenshotu, nikoli Product Profile.`
    : `Jsi kontrolní OCR nástroj. Vrať pouze JSON {"hourly_metrics":[{"hour":číslo nebo null,"product_code":"kód nebo null","availability_pct":číslo nebo null,"downtime_minutes":číslo nebo null,"downtime_before_production":true|false|null}]}. Čti pouze Dostupnost a Odstávku/Odstávky.`;
...
export const extractHourlyWithContext = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input: { imageDataUrl: string; context: HourlyStageContext }) => { if (!input?.imageDataUrl?.startsWith("data:image/")) throw new Error("Neplatný obrázek."); if (!input.context || !Number.isInteger(input.context.operator_count) || input.context.operator_count < 1) throw new Error("3. sekvence potřebuje skutečný počet operátorů."); if (!Array.isArray(input.context.profiles)) throw new Error("3. sekvence potřebuje Product Profile."); return input; }).handler(async ({ data }): Promise<HourlyStageResult> => {
  const dbProfiles = await loadExistingProfiles(data.context); const context = { ...data.context, profiles: dbProfiles };
  if (!(context.profiles ?? []).some((p) => (p.h_norm_per_hour != null && p.h_capacity != null) || (p.t_norm_per_hour != null && p.t_capacity != null))) throw new Error("Pro rozpoznané Product ID nebyl nalezen Product Profile s normou a kapacitou.");
  let parsed: Record<string, unknown> | null = null; let providerUsed: AiProvider | null = null; const errors: string[] = [];
  for (const provider of providers()) { try { parsed = await callAi(provider, data.imageDataUrl, context, "full"); providerUsed = provider; break; } catch (e) { errors.push(String(e)); } }
  if (!parsed || !providerUsed) throw new Error(`AI OCR se nepodařilo dokončit. ${errors.join("; ")}`);
  ...
  const hourly_metrics: HourlyStageMetric[] = raw.map((row) => {
    ...
    return {
      ...
      norm_per_hour: variant?.norm ?? null,
  ...
  if (missingAvailability) {
    try {
      ...
      const byHour = new Map<number, { availability: number | null; downtime: number | null; beforeProduction: boolean | null }>();
      for (const row of recoveryRows) {
        ...
        byHour.set(Math.round(h), { availability: a, downtime: d, beforeProduction: flag });
      }
      for (const metric of hourly_metrics) {
        const recovered = metric.hour != null ? byHour.get(Math.round(metric.hour)) : undefined;
  ...
  for (const m of hourly_metrics) {
    ...
    if (!isReal) {
      m.actual_minutes = 0;
      m.performance_pct = null;
      m.effective_norm = null;
    ...
    // First production hour: infer the production start only when there is NO concrete downtime.
    // teff = (OCR norm at 100% availability / Product Profile norm) * 60.
    ...
    if (canUseTeff) {
      ...
      m.actual_minutes = teff;
      m.effective_norm = m.norm_per_hour! * teff / 60;
    } else {
    ...
    if (m.effective_norm > 0 && m.actual_output != null) m.performance_pct = m.actual_output / m.effective_norm * 100;
    else m.performance_pct = null;
    if (m.performance_pct != null && availability != null && m.capacity != null && m.operator_count > 0) m.actual_oee_pct = m.performance_pct * availability * (m.capacity / m.operator_count) / 100;
    if (m.actual_output != null) actualOutputTotal += m.actual_output;
    if (m.effective_norm != null) idealOutputTotal += m.effective_norm;
    previousProduct = normalizedProduct;
  }
  ...
  return {
    ...
    actual_shift_performance_pct,
    actual_shift_availability_pct,
    operator_count: data.context.operator_count,
    screenshot_time,
    shift: shiftFromScreenshotTime(screenshot_time),
    raw: JSON.stringify({
      screenshot_time,
