    ...
    <AppShell title="Denní data" subtitle="Záznam se vytváří pouze když byl pracovník v práci – absence průměr OEE neovlivní.">
      <div className="space-y-5">
        <section className="grid gap-4 xl:grid-cols-2">
          <Card className="relative overflow-hidden border-border bg-card p-0 shadow-[var(--shadow-card)]"><div className="absolute inset-x-0 top-0 h-px bg-primary/50" /><div className="p-5 sm:p-6"><div className="mb-4 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl border border-primary/25 bg-primary/10 text-primary"><UploadCloud className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Import dat</p><h2 className="text-lg font-semibold">Screenshoty výroby</h2></div></div><div className="rounded-2xl border border-dashed border-border bg-muted/20 p-2"><ScreenshotImport employees={employees} /></div></div></Card>
          <Card className="relative overflow-hidden border-border bg-card p-0 shadow-[var(--shadow-card)]"><div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent" /><div className="flex h-full flex-col justify-between p-5 sm:p-6"><div><div className="mb-4 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl border border-primary/25 bg-primary/10 text-primary"><FilePlus2 className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Ruční zápis</p><h2 className="text-lg font-semibold">Zadat denní záznam ručně</h2></div></div><p className="max-w-xl text-sm leading-6 text-muted-foreground">Otevře se dialog s kompletním formulářem pro jeden denní záznam. Tabulka zůstává čistá a přehledná.</p></div><Button onClick={openNewManual} className="mt-6 w-full"><FilePlus2 className="mr-2 h-4 w-4" /> Zadat denní záznam ručně</Button></div></Card>
        </section>
      ...
      <Dialog open={detailOpen} onOpenChange={(open) => { setDetailOpen(open); if (!open) { setDetailRecord(null); setDetailHourly([]); setDetailImageUrl(null); } }}>
        <DialogContent className="max-h-[92vh] w-[calc(100%-1rem)] overflow-y-auto border-border/70 bg-background/95 sm:w-[calc(100%-3rem)] sm:max-w-5xl">
