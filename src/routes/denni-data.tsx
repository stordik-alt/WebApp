...
import { Download, FilePlus2, Pencil, Save, Search, Trash2, UploadCloud, X } from "lucide-react";
    ...
    <AppShell title="Denní data" subtitle="Záznam se vytváří pouze když byl pracovník v práci – absence průměr OEE neovlivní.">
      <div className="space-y-5">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          ].map((item) => <Card key={item.label} className="border-border/70 bg-slate-950/45 px-4 py-4"><p className="text-sm text-muted-foreground">{item.label}</p><div className={`mt-2 text-2xl font-semibold tabular-nums ${item.tone}`}>{item.value}</div><p className="mt-1 text-xs text-muted-foreground">{item.detail}</p></Card>)}
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-border/80 bg-slate-950/40 shadow-[var(--shadow-card)]">
          ...
          <div className="space-y-3 p-3 md:hidden">
            {filteredShifts.length === 0 ? <div className="rounded-xl border border-border/60 bg-slate-900/35 px-4 py-8 text-center text-sm text-muted-foreground">Zatím žádné záznamy pro zvolené filtry.</div> : filteredShifts.map((a) => <Card key={a.key} className="overflow-hidden border-border/70 bg-slate-900/35 p-0 shadow-none">
              ...
              <div className="grid grid-cols-2 gap-px border-b border-border/60 bg-border/50">{[{ label: "Ø OEE", value: `${fmt(a.oee)} %`, tone: metricTone(a.oee) }, { label: "Ø Výkon", value: `${fmt(a.performance)} %`, tone: "text-emerald-300" }, { label: "Ø Dostupnost", value: `${fmt(a.availableTime)} %`, tone: metricTone(a.availableTime) }, { label: "Výpomoc", value: fmt(a.help, 0), tone: "text-amber-200" }].map((metric) => <div key={metric.label} className="bg-slate-950/35 px-3 py-2.5"><p className="text-[10px] font-medium uppercase tracking-[0.1em] text-muted-foreground">{metric.label}</p><p className={`mt-0.5 text-sm font-semibold tabular-nums ${metric.tone}`}>{metric.value}</p></div>)}</div>
              <div className="space-y-2 p-3">{a.records.map((r) => <div key={r.id} className="rounded-xl border border-border/60 bg-slate-950/35 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-semibold">↳ {r.line}</p><p className="truncate text-xs text-muted-foreground">{r.product ?? "Produkt neuveden"}</p></div><div className="flex shrink-0 items-center gap-1"><Badge variant="outline" className={r.position === "HA" ? "border-cyan-400/40 text-cyan-300" : "border-fuchsia-400/40 text-fuchsia-300"}>{r.position}</Badge>{r.source === "screenshot" ? <Badge variant="outline" className="text-[9px]">import</Badge> : null}</div></div><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 border-t border-border/50 pt-2.5"><div><p className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">OEE</p><p className={`text-sm font-semibold tabular-nums ${metricTone(r.oee)}`}>{fmt(r.oee)} %</p></div><div><p className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">Výkon</p><p className="text-sm font-semibold tabular-nums text-emerald-300">{fmt(r.performance)} %</p></div><div><p className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">Dostupnost</p><p className={`text-sm font-semibold tabular-nums ${metricTone(r.available_time)}`}>{fmt(r.available_time)} %</p></div><div><p className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">Pozice</p><p className="text-sm text-foreground">{r.position}</p></div></div><div className="mt-3 flex justify-end gap-1 border-t border-border/50 pt-2"><Button size="sm" variant="ghost" onClick={() => loadForEdit(r)}><Pencil className="mr-1.5 h-3.5 w-3.5" /> Upravit</Button><Button size="sm" variant="ghost" onClick={() => remove.mutate(r.id)} disabled={remove.isPending}><Trash2 className="mr-1.5 h-3.5 w-3.5" /> Smazat</Button></div></div>)}</div>
            </Card>)}
          </div>

          <div className="flex flex-col gap-3 border-t border-border/70 px-4 py-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><span>Zobrazeno {filteredShifts.reduce((sum, a) => sum + a.records.length, 0)} záznamů ve {filteredShifts.length} směnových hodnoceních.</span><span className="text-muted-foreground/70">OEE a výkon mohou být nad 100 %.</span></div>
        </section>
      </div>
