...
export const Route = createFileRoute("/denni-data")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Rychlé zadávání denních záznamů: směna, linka, výrobek, pozice HA/TUP, OEE, výpomoc a spolupracovníci." },
      { property: "og:title", content: "Denní data – Výkonnost operátorů" },
      { property: "og:description", content: "Zadávání denních výkonových záznamů pracovníků výroby DPS." },
    ],
  }),
  component: DailyPage,
});
...
function DailyPage() {
  ...
  const canSave = workDate && shift && line.trim() && employeeId && !duplicateLine;
      ...
      <div className="grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-[420px_1fr]">
        ...
        <Card className="h-fit min-w-0 overflow-hidden gap-4 p-4 shadow-[var(--shadow-card)] sm:p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="grid gap-1.5">
              <Label>Linka</Label>
              <Input value={line} onChange={(e) => setLine(e.target.value)} placeholder="např. L1" />
            </div>
            <div className="grid gap-1.5">
              <Label>Výrobek</Label>
          ...
          {myShift ? (
            ...
            >
              {duplicateLine ? (
                <>Na lince <strong>{line.trim()}</strong> už tento pracovník v této směně záznam má – to je duplicita.</>
              ) : (
                <>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            ...
            >
              <Save className="h-4 w-4" /> Uložit záznam
            </Button>
            <Button
              variant="secondary"
              className="h-12"
              ...
              disabled={!canSave || create.isPending}
            >
              <Plus className="h-4 w-4" /> Uložit a přidat další
            </Button>
          </div>
        </Card>
        </div>
                  ...
                  <TableRow key={a.key} className="bg-muted/40">
                    ...
                    <TableCell>
                      <Badge variant={a.lineCount > 1 ? "default" : "secondary"}>
                        {a.lineCount} {a.lineCount === 1 ? "linka" : a.lineCount < 5 ? "linky" : "linek"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right font-semibold tabular-nums">
                      {fmt(a.oee)} %
                    </TableCell>
                    <TableCell className="text-right tabular-nums">{fmt(a.performance)} %</TableCell>
