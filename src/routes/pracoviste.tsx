...
import { Building2, ChevronDown, Factory, Pencil, Save, Users, X } from "lucide-react";
...
type Workplace = { id: string; code: string; line_name: string; workplace_name: string; area: "HA" | "TUP"; source_line: string | null; records: number; lastDate: string | null; avgOee: number | null };
...
function WorkplacesPage() {
  ...
  const { data: workplaces = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Workplace[]> => {
      ...
      const { data: records, error: recordsError } = await supabase.from("daily_records").select("line,work_date,oee");
      ...
      <div className="grid min-w-0 gap-6">
        <Card className="overflow-hidden p-0">
          <div className="border-b border-border px-4 py-4 sm:px-5">
            <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary"><Building2 className="h-5 w-5" /></span><div><h2 className="font-semibold">Seznam pracovišť</h2><p className="text-xs text-muted-foreground">{workplaces.length} pracovišť · řazeno podle kódu</p></div></div>
          </div>
