...
function ProductsPage() {
  ...
  const updateProduct = useMutation({
    mutationFn: async () => {
      ...
      const updates: Array<{ operation: "HA" | "TUP"; value: string }> = [
        { operation: "HA", value: editHaNorm },
        { operation: "TUP", value: editTupNorm },
      ];
      for (const item of updates) {
        ...
        if (current) {
          const { error } = await supabase.from("product_norms").update({ valid_to: validFrom }).eq("id", current.id);
          if (error) throw error;
        }
    ...
    onSuccess: () => { invalidate(); setEditProduct(null); toast.success("Produkt, kapacita a normy byly upraveny."); },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <div className="grid min-w-0 gap-6">
        ...
        <Card className="min-w-0 gap-3 overflow-hidden p-4 sm:p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="grid gap-1.5"><Label>Operace</Label><Select value={operation} onValueChange={(v) => setOperation(v as "HA" | "TUP")}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="HA">HA (celá linka)</SelectItem><SelectItem value="TUP">TUP</SelectItem></SelectContent></Select></div>
            <div className="grid gap-1.5"><Label>Norma (ks/h)</Label><Input type="number" step="0.1" className="h-11" value={normValue} onChange={(e) => setNormValue(e.target.value)} /></div>
            <div className="grid gap-1.5 sm:col-span-2"><Label>Platnost od</Label><Input type="date" className="h-11" value={validFrom} onChange={(e) => setValidFrom(e.target.value)} /></div>
          </div>
        ...
        {editProduct ? <Card className="min-w-0 gap-4 overflow-hidden p-4 sm:p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="grid gap-1.5"><Label>HA norma (ks/h)</Label><Input type="number" step="0.1" className="h-11" value={editHaNorm} onChange={(e) => setEditHaNorm(e.target.value)} /></div>
            <div className="grid gap-1.5"><Label>TUP norma (ks/h)</Label><Input type="number" step="0.1" className="h-11" value={editTupNorm} onChange={(e) => setEditTupNorm(e.target.value)} /></div>
          </div>
