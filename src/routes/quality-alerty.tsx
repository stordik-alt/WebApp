          ...
          {alerts.length === 0 ? (
            ...
            alerts.map((w) => (
              <div key={w.id} className={`rounded-xl border p-3 ${w.alert_resolved ? "border-border/60 bg-slate-950/35" : "border-destructive/30 bg-destructive/5"}`}>
                ...
                <div className="mt-3 flex justify-end border-t border-border/50 pt-2"><Button size="sm" variant="outline" onClick={() => setRow(w)}>{w.alert_resolved ? "Upravit" : "Vyšetřit"}</Button></div>
              </div>
            ))
          )}
        </div>
