-- Verze 2.0 AUTO
-- Atomic AUTO finalization + explicit duplicate accounting for batches.

create or replace function public.record_import_batch_duplicate(p_batch_id uuid, p_screenshot_path text default null)
returns void
language plpgsql
security invoker
...
begin
  ...
  set metadata = jsonb_set(
    ...
    '{duplicates}',
    to_jsonb(coalesce((metadata ->> 'duplicates')::integer, 0) + 1),
    true
  )
  where id = p_batch_id;
end;
...
grant execute on function public.record_import_batch_duplicate(uuid, text) to authenticated;
...
begin
  select coalesce((metadata ->> 'duplicates')::integer, 0)
    into v_duplicates
  ...
  set
    processed_items = least(b.total_items, counts.processed_items + v_duplicates),
    auto_items = counts.auto_items,
    pending_items = counts.pending_items,
    error_items = least(b.total_items, counts.error_items + v_duplicates)
  from (
    select
      ...
      count(*) filter (where status in ('ERROR','REJECTED'))::integer as error_items
...
begin
  ...
  if v_oee is null or not isfinite(v_oee::double precision) then
    v_reasons := v_reasons || '["HOURLY_KPI_MISSING"]'::jsonb;
  end if;
  ...
  if jsonb_array_length(v_reasons) > 0 then
    ...
    set product_id = v_product.id,
        ...
        updated_at = v_now
    where id = v_item.id;
    raise exception 'AUTO import nelze zpracovat: %', v_reasons using errcode = 'P0001';
  end if;
