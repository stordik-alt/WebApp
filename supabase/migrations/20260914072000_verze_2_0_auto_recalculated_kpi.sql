-- 2.0 AUTO: 3. + 4. sekvence – kanonický výpočet Výkonu a skutečného OEE
-- Výkon se NEBERE ze screenshotu. Vypočítá se z Reálného výstupu a Product Profile.
-- OEE používá takto vypočítaný Výkon + OCR Dostupnost.
...
begin
  ...
  loop
    ...
    -- 3. sekvence: Výkon = (Reálný výstup / norma z Product Profile) × 100.
    v_perf := case
    ...
    -- 4. sekvence: OEE = (Výkon × Dostupnost × (kapacita / počet operátorů)) / 100.
    v_oee := case
...
-- Po dokončení 1.–3. sekvence se KPI přepočítají ještě jednou na DB úrovni.
-- Tím je vyloučeno, aby chybná hodnota Výkonu ze screenshotu prošla do záznamu.
...
revoke all on function public.recalculate_import_item_kpis_on_validating() from public;
