...
begin
  ...
  for r in select h.* from public.import_item_hourly h where h.import_item_id = p_import_item_id order by h.hour, h.id loop
    v_norm := null; v_capacity := null;
    ...
    order by x.valid_to is null desc, x.valid_from desc nulls last, x.version_no desc nulls last limit 1;
    ...
    if v_downtime is null then v_downtime := nullif(coalesce(r.raw_data,'{}'::jsonb) ->> 'downtime_min','')::numeric; end if;
    if v_downtime is null and r.availability_pct is not null and r.availability_pct >= 0 and r.availability_pct < 100 and (coalesce(r.raw_data,'{}'::jsonb) ? 'availability_is_clock_time') then
    ...
    if v_norm is not null and v_norm > 0 and v_ocr_norm is not null and v_ocr_norm >= 0 and v_ocr_norm < v_norm then
      ...
      v_source := 'OCR_interval_norm_divided_by_master_norm'; v_status := 'DERIVED';
    elsif v_downtime > 0 then
      v_minutes := greatest(0, 60 - v_downtime);
      v_source := 'EXPLICIT_DOWNTIME'; v_status := 'DERIVED';
    else
      v_minutes := public.auto_shift_productive_minutes(v_item.shift, r.hour, v_item.ocr_data ->> 'screenshot_time');
      v_source := 'SHIFT_CLOCK_MODEL'; v_status := 'CLOCK';
    end if;

    v_expected := case when v_norm is not null and v_capacity is not null then v_norm * v_minutes / 60.0 * v_capacity / v_operator_count else null end;
    v_perf := case when r.actual_output is not null and v_norm is not null and v_norm > 0 and v_minutes > 0 then r.actual_output / (v_norm * v_minutes / 60.0) * 100 else null end;
    v_avail := case when v_downtime < 60 then greatest(0, least(100, (60 - v_downtime) / 60.0 * 100)) else 0 end;
    v_oee := case when v_perf is not null and v_capacity is not null and v_capacity > 0 then v_perf * v_avail * (v_capacity / v_operator_count::numeric) / 100.0 else null end;
