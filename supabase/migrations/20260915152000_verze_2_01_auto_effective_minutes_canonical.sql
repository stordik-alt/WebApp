-- Verze 2.01 AUTO: final canonical effective-time KPI persistence.
--
-- IMPORTANT:
--   * empty OCR rows (0/0 without product) are excluded;
--   * first real production hour may use teff only when there is no concrete downtime;
--   * after that, measured availability determines production minutes;
--   * a concrete downtime is relevant only before production or during a product
--     change, and availability is never reduced twice;
--   * actual_minutes is persisted so the UI cannot fall back to 60-minute weights.
...
begin
  ...
  loop
    -- Empty/non-production rows never participate in KPI or time weighting.
    ...
    -- OCR norm is the norm observed during the available part of the hour.
    -- Convert it to a 60-minute / 100%-availability norm first.
    v_norm_100 := case
    ...
    -- teff is used only as the inferred first production-hour duration.
    v_teff := case
    ...
    -- First real production hour: concrete downtime blocks teff.
    if v_is_first_productive
      ...
      -- Availability already contains most/all downtime in the source table;
      -- min() prevents subtracting the same downtime twice.
    ...
    set norm_per_hour = v_norm,
        ...
        raw_data = coalesce(raw_data, '{}'::jsonb) || jsonb_build_object(
          'calculation', jsonb_build_object(
            'model_version', '2.01-AUTO-effective-minutes-v3',
            'ocr_norm', v_ocr_norm,
            ...
            'performance_formula', '(Reálný výstup / (norma z Product Profile × efektivní výrobní čas / 60)) × 100',
            'effective_time_formula', '(Norma při 100% dostupnosti / norma z Product Profile) × 60 min',
            'downtime_rule', 'Konkrétní odstávka ovlivňuje výrobní čas pouze před začátkem výroby nebo při změně výrobku na stejném pracovišti. V první výrobní hodině blokuje teff, pokud není zároveň před začátkem výroby.',
  ...
  set ocr_data = jsonb_set(
    ...
    '{kpi_model_version}', to_jsonb('2.01-AUTO-effective-minutes-v3'::text), true
  )
...
-- Backfill every existing AUTO import using the final canonical function.
do $$
declare r record;
begin
  for r in
