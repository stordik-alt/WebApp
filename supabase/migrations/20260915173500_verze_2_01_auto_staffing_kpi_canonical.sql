-- Verze 2.01 AUTO: definitive staffing-aware KPI model.
--
-- Expected output:
--   norm_per_hour * productive_minutes / 60 * (operator_count / capacity)
-- Performance:
--   actual_output / expected_output * 100
-- OEE:
--   Performance * Availability * (operator_count / capacity) / 100
-- Quality is deliberately NOT part of OEE.
--
-- The staffing ratio is therefore applied to both the expected-output denominator
-- and the final OEE formula, exactly as defined by the 2.01 AUTO business rule.
  ...
  loop
    ...
    v_staffing_ratio := case
      when v_capacity is not null and v_capacity > 0 and v_operator_count > 0
        then v_operator_count / v_capacity
      else null
    end;

    v_expected := case
      when v_staffing_ratio is not null and v_staffing_ratio > 0
        then v_norm * v_minutes / 60.0 * v_staffing_ratio
      else null
    end;

    v_perf := case
      when r.actual_output is not null and v_expected is not null and v_expected > 0
        then (r.actual_output / v_expected) * 100.0
      else null
    end;

    v_oee := case
      when v_perf is not null and r.availability_pct is not null and v_staffing_ratio is not null
    ...
    set norm_per_hour = v_norm,
        ...
        raw_data = coalesce(raw_data, '{}'::jsonb) || jsonb_build_object(
          'calculation', jsonb_build_object(
            ...
            'expected_output', v_expected,
            'performance_formula', 'Reálný výstup / (norma × efektivní výrobní čas / 60 × počet operátorů / kapacita) × 100',
            'expected_output_formula', 'norma × efektivní výrobní čas / 60 × (počet operátorů / kapacita)',
            'oee_formula', 'Výkon × Dostupnost × (počet operátorů / kapacita) / 100',
