-- Verze 2.01 AUTO: fix same-hour product-change detection.
-- A product change is inferred only from a product present in an earlier hour.
-- This prevents two different products in the same hour from marking each other
-- as changed products. Availability remains authoritative for measured minutes.
...
begin
  ...
  for r in
    select h.id, h.hour, h.product_code, h.role, h.actual_output,
           h.availability_pct, h.norm_per_hour as stored_norm, h.raw_data
  ...
  loop
    ...
    -- Important: compare against the last product from an EARLIER hour only.
    -- Multiple products in one OCR hour must not mark one another as a changeover.
    if v_previous_hour is not null and r.hour > v_previous_hour then
      v_product_changed := v_previous_product is not null
        and lower(regexp_replace(coalesce(r.product_code, ''), '\\s+', '', 'g')) <> v_previous_product;
    else
      v_product_changed := false;
    end if;
    ...
    set norm_per_hour = v_norm,
        ...
        raw_data = coalesce(raw_data, '{}'::jsonb) || jsonb_build_object(
          'calculation', jsonb_build_object(
            'model_version', '2.01-AUTO-downtime-relevance-v3',
            'ocr_norm', v_ocr_norm,
    ...
    -- Keep the latest product as the reference for subsequent hours.
    -- Same-hour rows do not overwrite the reference until the hour changes.
    if v_previous_hour is null or r.hour > v_previous_hour then
  ...
  set ocr_data = jsonb_set(
    ...
    '{kpi_model_version}', to_jsonb('2.01-AUTO-downtime-relevance-v3'::text), true
  )
