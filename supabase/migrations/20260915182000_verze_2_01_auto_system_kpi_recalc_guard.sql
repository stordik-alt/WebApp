-- Verze 2.01 AUTO: robust guard for system KPI recalculation.
--
-- The KPI trigger cannot reliably identify a system recalculation from NEW.raw_data:
-- recalculate_import_item_kpis() updates import_item_rows before changing import_items.\n-- Use a transaction-local GUC as an explicit, non-persistent system-recalc marker.
...
begin
  ...
  is_system_recalc :=
    coalesce(current_setting('app.verze_2_01_auto_system_kpi_recalc', true), '') = 'on';
...
-- Replace the canonical function only to mark its own KPI write as system-owned.
-- The calculation body remains the canonical 2.01 AUTO effective-minutes implementation.
...
begin
  ...
  loop
    ...
    v_downtime_relevant := coalesce(v_downtime_minutes, 0) > 0 and (coalesce(v_downtime_before_production, false) or v_product_changed);
    ...
    -- Staffing is included in expected output and then again in the final OEE,
    -- exactly according to the approved 2.01 AUTO business rule.
    ...
    v_oee := case when v_perf is not null and r.availability_pct is not null and v_capacity is not null and v_capacity > 0 and v_operator_count > 0 then v_perf * r.availability_pct * (v_operator_count::double precision / v_capacity) / 100.0 else null end;

    update public.import_item_hourly
    set norm_per_hour = v_norm, capacity = v_capacity, operator_count = v_operator_count, actual_minutes = v_minutes,
        performance_pct = v_perf, actual_oee_pct = v_oee,
        raw_data = coalesce(raw_data, '{}'::jsonb) || jsonb_build_object('calculation', jsonb_build_object(
          'model_version','2.01-AUTO-staffing-kpi-v2','ocr_norm',v_ocr_norm,'norm_at_100_availability',v_norm_100,'effective_time_minutes',v_teff,'productive_minutes',v_minutes,'downtime_minutes',v_downtime_minutes,'downtime_before_production',v_downtime_before_production,'first_productive_row',v_is_first_productive,'product_changed',v_product_changed,'downtime_relevant',v_downtime_relevant,
          'performance_formula','Reálný výstup / (norma × teff × operátoři/kapacita) × 100','oee_formula','Performance × Dostupnost × (operátoři/kapacita) / 100','actual_output',r.actual_output,'norm_per_hour',v_norm,'availability_pct',r.availability_pct,'capacity',v_capacity,'operator_count',v_operator_count,'calculated_performance_pct',v_perf,'calculated_oee_pct',v_oee
        ))
    where id = r.id;
  ...
  set ocr_data = jsonb_set(jsonb_set(jsonb_set(coalesce(ocr_data, '{}'::jsonb), '{actual_shift_performance_pct}', to_jsonb(v_shift_perf), true), '{actual_shift_availability_pct}', to_jsonb(v_shift_avail), true), '{actual_shift_oee_pct}', to_jsonb(v_shift_oee), true),
      ocr_data = jsonb_set(ocr_data, '{kpi_model_version}', to_jsonb('2.01-AUTO-staffing-kpi-v2'::text), true)
...
-- Recalculate existing imports with the corrected canonical model.
do $$
declare r record;
begin
  for r in select distinct i.id from public.import_items i join public.import_item_hourly h on h.import_item_id = i.id where i.status in ('VALIDATING','PENDING_APPROVAL','AUTO_APPROVED','APPROVED','REJECTED')
  loop perform public.recalculate_import_item_kpis(r.id); end loop;
end;
$$;
