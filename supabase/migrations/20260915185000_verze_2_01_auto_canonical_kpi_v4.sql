-- Verze 2.01 AUTO: canonical KPI model v4.
-- Product Profile remains the canonical production norm/capacity.
-- OCR norm is preserved separately and is used only to reconstruct first-hour effective time.
...
-- Downtime is product-specific and never inferred from an unrelated product in the same hour.
...
begin
  ...
  loop
    -- A row with a known norm is a real production row even if its output is zero.
    -- Empty 0/0 rows remain excluded.
    ...
    v_product_changed := v_previous_product is not null
      ...
      and coalesce(upper(r.role), '') = coalesce(v_previous_role, '');

    -- Product Profile is the canonical norm/capacity, restricted to the import date.
    v_norm := null;
    v_capacity := null;
    select
      case
    ...
    -- Downtime relevance is explicitly product-specific. In particular, an
    -- unknown downtime cannot be transferred to another product in the same hour.
    v_downtime_relevant := coalesce(v_downtime_minutes, 0) > 0
      and (
        v_downtime_before_production = true
        or (v_product_changed and v_reason_is_changeover)
      );

    -- OCR norm remains separate from the Product Profile norm. It is only used
    -- to reconstruct the first real production row's effective time.
    v_norm_100 := case
    ...
    -- First real production row: teff is allowed only when there is no concrete
    -- downtime. A concrete downtime blocks teff even when it is not relevant.
    if v_is_first_productive
      ...
      -- Availability already contains downtime where applicable; min() prevents
      -- double subtraction.
    ...
    -- Explicit business rule: staffing ratio is actual operators / capacity.
    -- Do not cap the ratio; overstaffing may legitimately produce >100% OEE.
    v_staffing_ratio := v_operator_count::double precision / v_capacity;

    v_perf := case
      when r.actual_output is not null and v_norm > 0 and v_staffing_ratio > 0
        then r.actual_output / (v_norm * v_minutes / 60.0 * v_staffing_ratio) * 100.0
      else null
    end;

    v_oee := case
      when v_perf is not null and r.availability_pct is not null and v_staffing_ratio > 0
    ...
    set norm_per_hour = v_norm,
        ...
        raw_data = coalesce(raw_data, '{}'::jsonb) || jsonb_build_object(
          'calculation', jsonb_build_object(
            'model_version', '2.01-AUTO-staffing-kpi-v4',
            'ocr_norm', v_ocr_norm,
            ...
            'performance_formula', 'Reálný výstup / (Product Profile norma × teff × operátoři/kapacita) × 100',
  ...
  set ocr_data = jsonb_set(
    ...
    '{kpi_model_version}', to_jsonb('2.01-AUTO-staffing-kpi-v4'::text), true
  )
  where id = p_import_item_id;
end;
$$;
...
-- Recalculate existing imports using the corrected canonical function.
do $$
declare r record;
begin
  for r in
