-- Verze 2.01 AUTO: v6 product-specific downtime attribution.
-- A downtime marked as "before production" is relevant only when that row is
-- the first production row of the product / a product-change start.
-- This prevents a downtime flag recovered for another product in the same hour
-- from reducing production time of the wrong product.
...
begin
  ...
  loop
    ...
    -- Before-production downtime is product-specific. It may affect the first
    -- production row or a row starting a product change, but not a different
    -- product that merely shares the same hour.
    v_downtime_relevant := coalesce(v_downtime_minutes,0) > 0
      and ((v_downtime_before_production = true and (v_is_first_productive or v_product_changed))
    ...
    set norm_per_hour=v_norm,
        ...
        raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object('calculation',jsonb_build_object(
          'model_version','2.01-AUTO-staffing-kpi-v6',
          'ocr_norm',v_ocr_norm,
  ...
  set ocr_data=jsonb_set(
    ...
    ),'{kpi_model_version}',to_jsonb('2.01-AUTO-staffing-kpi-v6'::text),true
  )
