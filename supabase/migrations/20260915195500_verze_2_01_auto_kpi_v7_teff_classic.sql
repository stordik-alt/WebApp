-- Verze 2.01 AUTO V7
-- TEFF: OEE = Performance. Classic: OEE = Performance x Availability.
-- Staffing is applied once, in expected output.
...
declare
 ...
 v_downtime_minutes double precision; v_downtime_before boolean; v_reason text; v_changeover boolean; v_first boolean:=false; v_is_first boolean; v_previous_product text:=null; v_previous_role text:=null; v_changed boolean; v_relevant boolean; v_mode text;
begin
 ...
 for r in select h.id,h.hour,h.product_code,h.role,h.actual_output,h.availability_pct,h.raw_data from public.import_item_hourly h where h.import_item_id=p_import_item_id and not(coalesce(h.actual_output,0)=0 and coalesce(h.norm_per_hour,0)=0 and nullif(trim(coalesce(h.product_code,'')),'') is null) order by h.hour,h.id loop
  ...
  into v_norm,v_capacity from public.product_profiles pp where (lower(regexp_replace(coalesce(pp.ha_subassy,''),'\s+','','g'))=lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g')) or lower(regexp_replace(coalesce(pp.tup_subassy,''),'\s+','','g'))=lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g'))) and (v_work_date is null or pp.valid_from is null or pp.valid_from<=v_work_date) and (v_work_date is null or pp.valid_to is null or pp.valid_to>=v_work_date) order by pp.valid_to is null desc,pp.valid_from desc nulls last,pp.version_no desc nulls last limit 1;
  ...
  if v_relevant then v_mode:='TEFF'; v_minutes:=least(60.0,greatest(0,60.0-coalesce(v_downtime_minutes,0))); elsif v_is_first and coalesce(v_downtime_minutes,0)<=0 and v_teff is not null and v_teff>0 then v_mode:='TEFF'; v_minutes:=least(60.0,v_teff); else v_mode:='CLASSIC_AVAILABILITY'; v_minutes:=60.0; end if;
  ...
  update public.import_item_hourly set norm_per_hour=v_norm,capacity=v_capacity,operator_count=v_operator_count,actual_minutes=v_minutes,performance_pct=v_perf,actual_oee_pct=v_oee,raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object('calculation',jsonb_build_object('model_version','2.01-AUTO-staffing-kpi-v7','calculation_mode',v_mode,'ocr_norm',v_ocr_norm,'product_profile_norm',v_norm,'norm_at_100_availability',v_norm_100,'effective_time_minutes',v_teff,'productive_minutes',v_minutes,'downtime_minutes',v_downtime_minutes,'downtime_before_production',v_downtime_before,'downtime_reason',v_reason,'downtime_relevant',v_relevant,'first_productive_row',v_is_first,'product_changed',v_changed,'staffing_ratio',v_staffing_ratio,'performance_formula','actual / (Product Profile norm x time x staffing) x 100','oee_formula',case when v_mode='TEFF' then 'OEE = Performance' else 'OEE = Performance x Availability / 100' end)) where id=r.id;
