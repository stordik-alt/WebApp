-- Verze 2.01 AUTO V8
-- Product-finish TEFF rule: when the current product ends inside an hour,
-- use the full-line hourly board requirement (OCR norm) to reconstruct TEFF.
-- Staffing is applied once to expected output.
...
begin
 ...
 loop
  ...
  elsif v_is_first and coalesce(v_downtime_minutes,0)<=0 and v_teff is not null and v_teff>0 then
    v_mode:='TEFF'; v_minutes:=least(60.0,v_teff);
  else
  ...
  update public.import_item_hourly set norm_per_hour=v_norm,capacity=v_capacity,operator_count=v_operator_count,actual_minutes=v_minutes,performance_pct=v_perf,actual_oee_pct=v_oee,raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object('calculation',jsonb_build_object('model_version','2.01-AUTO-staffing-kpi-v8','calculation_mode',v_mode,'ocr_norm',v_ocr_norm,'product_profile_norm',v_norm,'norm_at_100_availability',v_norm_100,'effective_time_minutes',v_teff,'productive_minutes',v_minutes,'downtime_minutes',v_downtime_minutes,'downtime_before_production',v_downtime_before,'downtime_reason',v_reason,'downtime_relevant',v_relevant,'first_productive_row',v_is_first,'product_changed',v_changed,'product_finished',v_product_finished,'staffing_ratio',v_staffing_ratio,'performance_formula','actual / (Product Profile norm x time x staffing) x 100','oee_formula',case when v_mode='TEFF' then 'OEE = Performance' else 'OEE = Performance x Availability / 100' end)) where id=r.id;
