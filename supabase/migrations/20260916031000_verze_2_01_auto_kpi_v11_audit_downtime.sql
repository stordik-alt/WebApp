-- Verze 2.01 AUTO V11
-- Audit correction for multi-product TEFF.
-- A downtime recorded after a product has already finished must not be marked
-- relevant to that completed product. Changeover downtime belongs to the
-- following product in the same hour/workplace sequence.
--
-- This migration intentionally leaves V10's TEFF-chain calculation intact.
-- It only normalizes the audit metadata so the stored calculation explains
-- which downtime actually affected the allocated productive time.
...
declare
 ...
 v_changed boolean; v_relevant boolean; v_mode text; v_product_finished boolean;
 v_same_hour boolean; v_next_same_role boolean; v_time_used double precision:=0;
begin
 ...
 loop
  ...
  v_same_hour:=r.next_hour is not null and r.next_hour=r.hour;
  v_next_same_role:=v_same_hour and upper(coalesce(r.next_role,''))=upper(coalesce(r.role,''));
  ...
  -- Only downtime before production or explicitly identified changeover
  -- downtime can affect the current product. Downtime after a finished product
  -- is intentionally not relevant to that completed product.
  ...
  update public.import_item_hourly set norm_per_hour=v_norm,capacity=v_capacity,operator_count=v_operator_count,actual_minutes=v_minutes,performance_pct=v_perf,actual_oee_pct=v_oee,raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object('calculation',coalesce(raw_data->'calculation','{}'::jsonb)||jsonb_build_object('model_version','2.01-AUTO-staffing-kpi-v11','calculation_mode',v_mode,'ocr_norm',v_ocr_norm,'product_profile_norm',v_norm,'norm_at_100_availability',v_norm_100,'effective_time_minutes',v_teff,'productive_minutes',v_minutes,'downtime_minutes',v_downtime_minutes,'downtime_before_production',v_downtime_before,'downtime_reason',v_reason,'downtime_relevant',v_relevant,'first_productive_row',v_is_first,'product_changed',v_changed,'product_finished',v_product_finished,'same_hour_next_product',v_product_finished,'staffing_ratio',v_staffing_ratio,'time_allocation_rule','TEFF first product = OCR expected norm / Product Profile norm x 60; following product = remaining 60 min minus preceding TEFF and applicable changeover downtime','performance_formula','actual / (Product Profile norm x time x staffing) x 100','oee_formula',case when v_mode='TEFF' then 'OEE = Performance' else 'OEE = Performance x Availability / 100' end)) where id=r.id;
