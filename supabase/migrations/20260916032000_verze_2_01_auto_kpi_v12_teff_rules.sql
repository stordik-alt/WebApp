-- Verze 2.01 AUTO V12
-- TEFF is used only in two situations:
-- 1) production starts after the canonical shift-start hour and there is
--    documented downtime before that first production;
-- 2) one clock hour contains multiple products on the same role/workplace.
-- All other hours use CLASSIC_AVAILABILITY.
-- In a multi-product hour, TEFF is chained: first product from OCR hourly norm,
-- each following product receives the remaining hour minus applicable changeover.
-- Staffing is applied exactly once in expected output.

create or replace function public.recalculate_import_item_kpis(p_import_item_id uuid)
returns void language plpgsql security definer set search_path=public as $$
declare
 v_operator_count integer; v_work_date date;
 ...
 r record;
 v_minutes double precision; v_norm double precision; v_ocr_norm double precision; v_norm_100 double precision;
 v_capacity double precision; v_teff double precision; v_perf double precision; v_oee double precision; v_staffing_ratio double precision;
 v_downtime_minutes double precision; v_downtime_before boolean; v_reason text; v_changeover boolean;
 v_changed boolean; v_relevant boolean; v_mode text; v_product_finished boolean; v_is_first_in_hour boolean;
 v_is_first_in_shift boolean; v_seen_in_shift boolean:=false; v_hour_minutes_used double precision:=0;
 v_previous_hour integer:=null; v_previous_role text:=null; v_previous_product text:=null;
 v_time_remaining_before double precision; v_changeover_downtime double precision; v_teff_source text; v_formula text;
begin
 ...
 loop
  select
   case when upper(coalesce(r.role,''))='TUP' and lower(regexp_replace(coalesce(pp.tup_subassy,''),'\s+','','g'))=lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g')) then pp.t_norm_per_hour else pp.h_norm_per_hour end,
  ...
  -- Reset both the hourly chain and the shift-start tracker at canonical shift boundaries.
  if r.hour in(6,14,22) then v_seen_in_shift:=false; end if;
  v_is_first_in_hour:=v_previous_hour is distinct from r.hour or upper(coalesce(v_previous_role,''))<>upper(coalesce(r.role,''));
  if v_is_first_in_hour then v_hour_minutes_used:=0; end if;
  v_is_first_in_shift:=not v_seen_in_shift;
  v_seen_in_shift:=true;
  ...
  v_downtime_minutes:=greatest(0,least(60,coalesce(nullif(trim(coalesce(r.raw_data->>'downtime_minutes','')),'')::double precision,nullif(trim(coalesce(r.raw_data->>'downtime_min','')),'')::double precision,nullif(trim(coalesce(r.raw_data->>'odstavka_minutes','')),'')::double precision,nullif(trim(coalesce(r.raw_data->>'odstavka_min','')),'')::double precision,0)));
  ...
  -- TEFF trigger #1: production really starts after shift start.
  -- Canonical shift starts are 06:00, 14:00 and 22:00, represented by hours 6/14/22.
  -- A first production in hour 6/14/22 therefore remains classic even if it has downtime.
  v_relevant:=v_downtime_minutes>0 and v_is_first_in_shift and r.hour not in(6,14,22) and v_downtime_before=true;
  ...
  -- TEFF trigger #2: multiple products in one clock hour.
  if v_product_finished and v_ocr_norm is not null and v_ocr_norm>0 then
    ...
    v_teff_source:='MULTI_PRODUCT_FIRST';
    v_formula:='TEFF = (OCR norm / Product Profile norm) x 60';
    v_mode:='TEFF'; v_minutes:=v_teff; v_relevant:=false;

  elsif v_previous_hour=r.hour and v_previous_role is not distinct from upper(coalesce(r.role,'')) and v_changed then
    v_time_remaining_before:=greatest(0,60-v_hour_minutes_used);
    v_changeover_downtime:=case when v_changeover then v_downtime_minutes else 0 end;
    v_teff:=greatest(0,least(v_time_remaining_before,v_time_remaining_before-v_changeover_downtime));
    v_teff_source:=case when v_changeover_downtime>0 then 'MULTI_PRODUCT_REMAINDER_MINUS_CHANGEOVER' else 'MULTI_PRODUCT_REMAINDER' end;
    v_formula:='TEFF = remaining hour - preceding TEFF total - changeover downtime';
    v_mode:='TEFF'; v_minutes:=v_teff; v_relevant:=v_changeover_downtime>0;

  elsif v_relevant then
    -- Late start outside the shift boundary: TEFF is the actual available production time.
    v_teff:=greatest(0,60-v_downtime_minutes);
    v_teff_source:='LATE_PRODUCTION_START';
    v_formula:='TEFF = 60 - downtime before production';
    v_mode:='TEFF'; v_minutes:=v_teff;

  else
    v_teff:=null; v_teff_source:='CLASSIC_FULL_HOUR'; v_formula:='Classic availability calculation';
    v_mode:='CLASSIC_AVAILABILITY'; v_minutes:=60; v_relevant:=false;
  end if;

  v_minutes:=least(60,greatest(0,v_minutes));
  v_hour_minutes_used:=least(60,v_hour_minutes_used+v_minutes);
  v_staffing_ratio:=v_operator_count::double precision/v_capacity;
  v_perf:=case when r.actual_output is not null and v_staffing_ratio>0 and v_minutes>0 then r.actual_output/(v_norm*v_minutes/60*v_staffing_ratio)*100 else null end;
  v_oee:=case when v_perf is null then null when v_mode='TEFF' then v_perf when r.availability_pct is not null then v_perf*r.availability_pct/100 else null end;
  ...
  update public.import_item_hourly set norm_per_hour=v_norm,capacity=v_capacity,operator_count=v_operator_count,actual_minutes=v_minutes,performance_pct=v_perf,actual_oee_pct=v_oee,
   raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object('calculation',coalesce(raw_data->'calculation','{}'::jsonb)||jsonb_build_object(
    'model_version','2.01-AUTO-staffing-kpi-v12','calculation_mode',v_mode,'ocr_norm',v_ocr_norm,'product_profile_norm',v_norm,
    'norm_at_100_availability',v_norm_100,'effective_time_minutes',v_teff,'productive_minutes',v_minutes,
    'downtime_minutes',v_downtime_minutes,'downtime_before_production',v_downtime_before,'downtime_reason',v_reason,
    'downtime_relevant',v_relevant,'first_productive_row_in_shift',v_is_first_in_shift,'first_productive_row_in_hour',v_is_first_in_hour,
    'product_changed',v_changed,'product_finished',v_product_finished,'staffing_ratio',v_staffing_ratio,'teff_source',v_teff_source,
    'teff_formula',v_formula,'shift_start_hours',jsonb_build_array(6,14,22),
    'teff_rule','TEFF only for late production start outside shift-start hour or multi-product clock hour',
    'performance_formula','actual / (Product Profile norm x time x staffing) x 100',
    'oee_formula',case when v_mode='TEFF' then 'OEE = Performance' else 'OEE = Performance x Availability / 100' end)) where id=r.id;
  ...
  v_previous_hour:=r.hour;v_previous_product:=lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g'));v_previous_role:=upper(coalesce(r.role,''));
 end loop;
