-- Verze 2.01 AUTO V13
-- Fixed shift breaks are part of the canonical time model and must always be
-- excluded from available production time. The existing shift clock function
-- is authoritative for scheduled productive minutes and also preserves the
-- already-defined 7 min setup / 5 min cleanup boundaries.
--
-- Ranní:     06:00-14:00, break 10:40-11:10
-- Odpolední: 14:00-22:00, break 18:00-18:30
-- Noční:     22:00-06:00, break 02:00-02:30
--
-- This migration changes only the canonical recalculation function on
-- Verze-2.01-Auto. main is untouched.
...
declare
 ...
 v_same_hour boolean; v_next_same_role boolean; v_remaining_minutes double precision;
begin
 ...
 loop
  ...
  -- Fixed breaks/setup/cleanup are scheduled non-production time, not downtime.
  -- The helper returns the exact productive minutes available in this clock hour.
  v_scheduled_minutes:=public.auto_shift_productive_minutes(v_shift,r.hour,v_screenshot_time);

  -- Only downtime before production or explicitly identified changeover downtime
  -- can affect allocated production time. Downtime after a finished product is
  -- not charged to that completed product.
  ...
  -- TEFF is used only for a production that finishes inside a multi-product
  -- hour. The first product receives its proportional share of the scheduled
  -- productive time; the following product receives the remaining scheduled
  -- time, less applicable changeover downtime.
  if v_product_finished and v_ocr_norm is not null and v_ocr_norm>0 and v_scheduled_minutes>0 then
    v_mode:='TEFF';
    v_teff:=least(v_scheduled_minutes,greatest(0,(v_ocr_norm/v_norm)*v_scheduled_minutes));
    v_minutes:=v_teff;
  elsif v_next_same_role and v_changed and v_scheduled_minutes>0 then
    v_mode:='TEFF';
    v_remaining_minutes:=greatest(0,v_scheduled_minutes);
    -- Previous TEFF is already represented by earlier rows in the same hour.
    -- Calculate the remainder from the canonical scheduled hour and preceding
    -- rows rather than using a calendar break as downtime.
    select greatest(0, v_scheduled_minutes - coalesce(sum(coalesce(x.actual_minutes,0)),0))
      into v_remaining_minutes
    from public.import_item_hourly x
    where x.import_item_id=p_import_item_id and x.hour=r.hour and x.id<>r.id;
  ...
  update public.import_item_hourly set
    ...
    raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object(
      'calculation',coalesce(raw_data->'calculation','{}'::jsonb)||jsonb_build_object(
        'model_version','2.01-AUTO-staffing-kpi-v13',
        ...
        'product_finished',v_product_finished,'same_hour_next_product',v_product_finished,
        'staffing_ratio',v_staffing_ratio,
        'time_allocation_rule','TEFF first product = OCR expected norm / Product Profile norm x scheduled productive minutes; following product = remaining scheduled productive minutes minus applicable changeover downtime',
