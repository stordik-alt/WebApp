-- Verze 2.01 AUTO V14
-- Canonical KPI recalculation with fixed shift breaks and a complete TEFF chain.
--
-- Fixed breaks are scheduled non-production time and are never treated as
-- downtime:
-- Ranní     06:00-14:00, 10:40-11:10
-- Odpolední 14:00-22:00, 18:00-18:30
-- Noční     22:00-06:00, 02:00-02:30
--
-- TEFF is used only for a multi-product hour, or for qualifying downtime at
-- the beginning of production outside the shift start. In a multi-product
-- hour the first product receives its proportional share of scheduled
-- productive minutes and every following product receives the remaining
-- productive minutes. The final product is included in this chain.
-- main is untouched.
...
begin
 ...
 loop
  ...
  -- Fixed breaks, shift setup and cleanup are scheduled non-production time.
  -- auto_shift_productive_minutes returns only the minutes available for
  -- production in the clock hour, including the fixed break subtraction.
  ...
  if v_multi_product_hour and v_is_first and v_ocr_norm is not null and v_ocr_norm>0 and v_scheduled_minutes>0 then
    -- First product: proportional share of the scheduled productive time.
    v_mode:='TEFF';
    ...
    -- Every following product, including the final one, receives the
    -- remaining scheduled productive time in this hour.
    v_mode:='TEFF';
    select greatest(0,
  ...
  update public.import_item_hourly set
    ...
    raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object(
      'calculation',coalesce(raw_data->'calculation','{}'::jsonb)||jsonb_build_object(
        'model_version','2.01-AUTO-staffing-kpi-v14',
