-- Verze 2.01 AUTO V17
-- Correct productive-time calculation and multi-product TEFF allocation.
-- Scope: Verze-2.01-Auto only. main is untouched.
--
-- The canonical shift helper is intentionally used with its existing
-- 3-argument signature. The fourth-argument variant was not canonical.
-- The screenshot header time remains authoritative for the partial final hour.
-- Fixed breaks are scheduled non-production time and are subtracted only
-- by their actual overlap with the clock hour.
--
-- IMPORTANT KPI rules:
-- 1) TEFF only for a multi-product hour, or qualifying downtime at the
--    beginning of production outside shift start.
-- 2) In a multi-product hour, first product gets:
--      OCR norm / Product Profile norm * scheduled productive minutes.
-- 3) Each following product gets the remaining scheduled productive minutes
--    after the previous allocations and applicable product-change downtime.
-- 4) Staffing is applied once in expected output.
-- 5) Classic rows use availability; TEFF rows have OEE = Performance.
...
DECLARE
  ...
  v_scheduled_minutes double precision;
...
BEGIN
  ...
  LOOP
    SELECT
      CASE
        WHEN upper(coalesce(r.role,''))='TUP'
         AND lower(regexp_replace(coalesce(pp.tup_subassy,''),'\s+','','g'))=
             lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g'))
    ...
    v_is_first := NOT v_first;
    v_first := true;

    v_same_hour := v_previous_hour IS NOT NULL AND v_previous_hour=r.hour;
    v_next_same_role := r.next_hour IS NOT NULL
      AND r.next_hour=r.hour
      AND upper(coalesce(r.next_role,''))=upper(coalesce(r.role,''));

    v_changed := v_same_hour
      AND upper(coalesce(r.role,''))=upper(coalesce(v_previous_role,''))
      AND lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g'))
          <> coalesce(v_previous_product,'');

    v_product_finished := v_next_same_role
      AND lower(regexp_replace(coalesce(r.next_product_code,''),'\s+','','g'))
          <> lower(regexp_replace(coalesce(r.product_code,''),'\s+','','g'));
    ...
    -- The 3-argument helper is canonical in this branch and uses the
    -- screenshot header time to shorten the final partial clock hour.
    ...
    -- Unknown/irrelevant downtime after a completed product is never charged
    -- to that completed product. Product-change downtime belongs to the next
    -- product in the same hour.
    v_relevant := coalesce(v_downtime_minutes,0)>0
      AND (
        (v_downtime_before=true AND (v_is_first OR v_changed))
    ...
    IF v_multi_product_hour AND v_is_first
       AND v_ocr_norm IS NOT NULL AND v_ocr_norm>0
       AND v_scheduled_minutes>0 THEN
      v_mode := 'TEFF';
      v_minutes := LEAST(
        v_scheduled_minutes,
        GREATEST(0,(v_ocr_norm/v_norm)*v_scheduled_minutes)
      );
    ELSIF v_multi_product_hour AND v_changed AND v_scheduled_minutes>0 THEN
      ...
      IF v_changeover THEN
        v_remaining_minutes := GREATEST(
          0, v_remaining_minutes-coalesce(v_downtime_minutes,0)
        );
      END IF;
    ...
    SET norm_per_hour=v_norm,
        ...
        raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object(
          'calculation',coalesce(raw_data->'calculation','{}'::jsonb)||jsonb_build_object(
            'model_version','2.01-AUTO-staffing-kpi-v17',
            'calculation_mode',v_mode,
            'ocr_norm',v_ocr_norm,
            'product_profile_norm',v_norm,
            'scheduled_productive_minutes',v_scheduled_minutes,
            'effective_time_minutes',CASE WHEN v_mode='TEFF' THEN v_minutes ELSE NULL END,
            ...
            'fixed_break_excluded',true,
            'fixed_break_rule','Ranní 10:40-11:10; Odpolední 18:00-18:30; Noční 02:00-02:30',
            'first_productive_row',v_is_first,
            'product_changed',v_changed,
            'multi_product_hour',v_multi_product_hour,
            'product_finished',v_product_finished,
            'staffing_ratio',v_staffing_ratio,
            'time_allocation_rule','TEFF first product = OCR norm / Product Profile norm x scheduled productive minutes; following product = remaining scheduled productive minutes minus applicable changeover downtime',
  ...
  SET performance=v_shift_perf,
      ...
      raw_data=coalesce(raw_data,'{}'::jsonb)||jsonb_build_object(
        ...
        'available_time',v_shift_avail,
        'oee',v_shift_oee,
        'kpi_model_version','2.01-AUTO-staffing-kpi-v17'
      )
  WHERE import_item_id=p_import_item_id
  ...
  SET ocr_data=jsonb_set(
    ...
    '{kpi_model_version}',to_jsonb('2.01-AUTO-staffing-kpi-v17'::text),true)
  WHERE id=p_import_item_id;
END;
$$;
