-- Verze 2.01 AUTO V18.2
-- Fix staffing direction and KPI chain without changing the validated
-- Product Profile lookup or CLASSIC/TEFF time-model structure.
-- Staffing is applied exactly once in Expected Output.
-- Availability preserves an existing source value when present.
...
begin
  ...
  loop
    ...
    -- Canonical Product Profile remains authoritative.
    ...
    -- CLASSIC = one product in the hour; TEFF = multiple products.
    ...
    if not v_is_teff then
      -- CLASSIC: OCR norm does not determine productive minutes.
      ...
      -- TEFF: preserve the validated OCR interval reconstruction.
    ...
    -- V18.2 FIX: staffing = operators / capacity, applied once here.
    v_expected := case
    ...
    -- V18.2 FIX: Performance is actual output versus staffed Expected Output.
    v_perf := case
    ...
    -- Preserve an existing source Availability. Only derive it from downtime
    -- when no source Availability exists.
    v_avail := case
    ...
    -- V18.2 FIX: staffing is NOT applied again in OEE.
    v_oee := case
      ...
      raw_data = coalesce(raw_data,'{}'::jsonb)
        || jsonb_build_object(
          ...
          || jsonb_build_object(
            ...
            'capacity', v_capacity,
            'operator_count', v_operator_count,
            'staffing_factor', case
              when v_capacity is not null and v_capacity > 0
              then v_operator_count::numeric / v_capacity
              else null
            end,
