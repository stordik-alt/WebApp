...
-- Existing Availability remains authoritative when already populated.
-- TEFF multi-product hours allocate the hour sequentially and never exceed 60 minutes.
...
begin
  ...
  for r in
    ...
    select
      h.*,
      row_number() over (partition by h.hour order by h.id) as hour_seq,
      hc.product_count
    from public.import_item_hourly h
  ...
  loop
    ...
    v_downtime := 0;
    v_minutes := null;
    v_downtime_before_production := false;
    v_is_last_hour := (r.hour = v_last_hour);

    if r.hour_seq = 1 then
      v_hour_allocated := 0;
    end if;
    ...
    v_downtime_before_production := lower(coalesce(r.raw_data ->> 'downtime_before_production','false')) in ('true','1','yes','y','ano');
    v_hour_product_count := coalesce(r.product_count, 0);
    v_is_teff := v_hour_product_count > 1;

    if not v_is_teff then
      -- CLASSIC: OCR norm is not a time source in a single-product hour.
      ...
      -- Downtime before production start reduces productive time.
      -- Downtime after production must not retroactively penalize the completed product.
      if v_downtime_before_production and v_downtime > 0 then
        v_minutes := greatest(0, v_minutes - v_downtime);
        v_source := 'SHIFT_CLOCK_MODEL_CLASSIC_MINUS_PREPRODUCTION_DOWNTIME';
        v_status := 'DERIVED';
      else
      ...
      -- TEFF: first product gets its OCR-derived interval when available.
      -- Later products consume only the remaining time in the same hour.
      if r.hour_seq = 1 then
        ...
        then
          ...
          if v_downtime_before_production and v_downtime > 0 then
            v_minutes := greatest(0, v_minutes - v_downtime);
            v_source := 'SHIFT_CLOCK_MODEL_TEFF_FIRST_MINUS_PREPRODUCTION_DOWNTIME';
            v_status := 'DERIVED';
          else
            v_source := 'SHIFT_CLOCK_MODEL_TEFF_FIRST_FALLBACK';
            v_status := 'CLOCK';
          end if;
        end if;
      else
        v_minutes := greatest(0, least(60 - v_hour_allocated, 60 - v_hour_allocated));
        if v_downtime > 0 then
          v_minutes := greatest(0, v_minutes - v_downtime);
          v_source := 'TEFF_REMAINING_TIME_MINUS_CHANGEOVER_DOWNTIME';
        else
          v_source := 'TEFF_REMAINING_TIME';
        end if;
        v_status := 'DERIVED';
      end if;
    end if;

    v_minutes := greatest(0, least(60 - v_hour_allocated, coalesce(v_minutes, 0)));
    ...
    if v_is_teff then
      v_hour_allocated := least(60, v_hour_allocated + v_minutes);
    else
      v_hour_allocated := v_minutes;
    end if;
  end loop;
end;
$$;
