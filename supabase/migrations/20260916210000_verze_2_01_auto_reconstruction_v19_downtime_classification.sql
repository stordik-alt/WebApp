-- Verze 2.01 AUTO – V19 KPI reconstruction.
-- Replaces V18.2's CLASSIC/TEFF split with a downtime-driven model, per audit
-- of the 041.05-OLOVO test screenshot (2026-08-19):
--
-- TEFF (time derived from this row's own OCR-read norm, ocr_norm/PP_norm*60)
-- is used as the time basis whenever ANY of these hold for a row's hour:
--   - it is a multi-product hour (product changed within the hour), or
--   - it is the first hour with real output in the shift and that hour is
--     not the shift's start hour ("late start", no downtime info exists to
--     say how long the line was idle), or
--   - the row's own downtime reason classifies as "Neovlivnitelná"
--     (uncontrollable: equipment/process faults, missing frames, material
--     handling/replenishment, a production/product change) even in an
--     otherwise normal, on-time, single-product hour.
-- If, in one of those TEFF cases, the row's downtime instead classifies as
-- "Ovlivnitelná" (controllable: WC/drink breaks), its minutes are ADDED on
-- top of the OCR-derived TEFF minutes (it does affect Expected Output).
-- Otherwise (normal hour, no or only controllable downtime, single product,
-- on time) CLASSIC shift-clock time is used unchanged, and any controllable
-- downtime is ignored for calculation purposes.
--
-- The downtime-reason classification is a fuzzy, diacritic-insensitive
-- keyword match (exact screenshot wording is not reliable/remembered) and is
-- hardcoded for now; a maintainable list + admin UI is a separate follow-up.
...
begin
  ...
  loop
    ...
    -- Fuzzy, diacritic-insensitive classification of the downtime reason text.
    v_downtime_reason_norm := lower(translate(
      coalesce(r.raw_data ->> 'downtime_reason', ''),
      'ěščřžýáíéůúňťďóĚŠČŘŽÝÁÍÉŮÚŇŤĎÓ',
      'escrzyaieuuntdoESCRZYAIEUUNTDO'
    ));

    v_downtime_category := case
      when v_downtime_reason_norm = '' or v_downtime = 0 then 'NONE'
      when v_downtime_reason_norm ~ 'porucha|zmena vyroby|zmena produktu|nedostatek ramecku|manipulace|dopln' then 'UNCONTROLLABLE'
      when v_downtime_reason_norm ~ '(^|[^a-z])wc([^a-z]|$)|piti' then 'CONTROLLABLE'
      else 'UNCLASSIFIED'
    end;
    ...
    -- Staffing = actual operators / product capacity.
    ...
    -- Performance = Actual / Expected. No second staffing correction here.
    v_perf := case
    ...
    -- Availability is an independent source metric. Preserve it when present.
    v_avail := case
    ...
    -- OEE = Performance × Availability. Staffing is not applied again.
    v_oee := case
