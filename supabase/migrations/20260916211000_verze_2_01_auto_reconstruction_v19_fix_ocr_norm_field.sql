-- Fix: the V19 migration above read raw_data->>'norm_per_hour' for the OCR
-- norm, but that field actually holds a stale, non-varying leftover value
-- (39 for every H_S4572A-IA row, 64 for every H_SM11447 row, regardless of
-- hour) from an earlier pipeline stage — not the true per-hour OCR reading.
-- The real per-hour value the OCR actually captured (matching the "norma"
-- column on the screenshot exactly: 12, 15, 9, 87, 87, 65, 78, 45 across
-- this test import's hours) is raw_data->>'ocr_norm_per_hour'. Verified via
-- SELECT audit before applying this fix.
