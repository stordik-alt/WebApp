-- Replace the hardcoded downtime-reason regex in reconstruct_import_item_hourly
-- (V19) with a lookup against the new, editable downtime_reason_classifications
-- table via classify_downtime_reason(). Behavior is unchanged for reasons
-- already seeded (verified by re-running the test import below); new reasons
-- can now be classified going forward from the admin UI instead of requiring
-- a code change.
