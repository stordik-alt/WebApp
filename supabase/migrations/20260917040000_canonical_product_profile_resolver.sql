-- Root-cause fix for BUG-01/02/03 from the "Koncept pro agenta" analysis:
-- product identification previously only checked products.code for an exact
-- string match (in JS import-v2-auto.ts AND in both SQL legacy approval
-- functions, independently and inconsistently). It never consulted
-- product_profiles.ha_subassy/tup_subassy, so any OCR-read code that only
-- exists as a profile pairing (confirmed: 79 of 290 distinct tup_subassy
-- values in product_profiles have no matching products row at all) was
-- always reported PRODUCT_NOT_FOUND regardless of correctness.
--
-- This is the one canonical resolver both SQL approval paths now call.
-- Order of resolution:
--   1) exact match against products.code (existing behavior, kept first
--      since it is the cheapest and most certain).
--   2) match against product_profiles.ha_subassy/tup_subassy, valid at the
--      given work date. If the matched side's code also has a products row,
--      that product is used together with this profile.
--   3) last resort: a single trailing 1-3 letter suffix (covers the real
--      B/U/UCS variant-letter convention seen in tup_subassy, e.g.
--      T_S4962V3422B vs T_S4962V3422U) is stripped ONLY when the exact
--      suffixed code has no footprint anywhere (steps 1-2 both found
--      nothing) AND the stripped base code does resolve. This never
--      fires for a code that is itself a real, separately registered
--      variant, since such a code would already resolve in step 1 or 2.
--      Fallback recursion is capped at one strip (p_allow_fallback) so it
--      cannot chain into over-stripping a genuinely unrelated code.
...
begin
  ...
  if v_product.id is not null and v_profile.id is not null then
    return query select v_product.id, v_product.code, v_product.name, v_profile.id,
      ...
      (v_profile.ha_subassy is not null and v_profile.tup_subassy is not null
        and v_profile.h_norm_per_hour > 0 and v_profile.h_capacity >= 1
        and v_profile.t_norm_per_hour > 0 and v_profile.t_capacity >= 1),
      'EXACT'::text;
    return;
  end if;
