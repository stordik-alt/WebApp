-- Wire the canonical public.resolve_product_profile() resolver (added in
-- 20260917040000) into both legacy approval functions, replacing their
-- independent inline products.code-only lookups. This is the second half of
-- the BUG-01/02/03 fix: identification now checks product_profiles.ha_subassy
-- /tup_subassy (with the same conservative single-letter-suffix fallback)
-- everywhere a product is resolved for approval, not just at OCR-import time.
