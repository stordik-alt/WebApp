-- Master Prompt Problem 11: productive-time calculation at the start/end of
-- a production run inside a shift.
--
-- ROOT CAUSE (confirmed by reading the live function before touching it):
-- reconstruct_import_item_hourly() only ever recognized the single overall
-- FIRST hour with output (min(hour) where actual_output>0) and the single
-- overall LAST recorded hour (max(hour)) of an import_item as needing
-- special start/end treatment. A shift can contain MORE THAN ONE separate
-- production run - e.g. run 1 ends at hour 8, hour 9 is a genuine gap
-- (0 output, no product), run 2 starts at hour 10 - and neither hour 8 nor
-- hour 10 is the item's single global first/last hour, so both fell through
-- to the plain shift-clock model and were credited a flat, unadjusted hour
-- (60 minutes for the segment end, or 60-minus-break for the segment start)
-- regardless of when production actually stopped/started. This also used
-- raw hour-of-day for ordering/min/max instead of shift-relative hour, which
-- would additionally misorder a night shift wrapping past midnight.
--
-- FIX: segment boundaries are now detected PER ROW by looking at its actual
-- neighbors (previous/next hour by shift-relative order), not via two
-- precomputed scalars for the whole item:
--   - is_segment_start: the previous shift-relative hour is missing from
--     the recorded rows, or had zero output - and this isn't the shift's
--     very first hour (which the CLASSIC model already handles correctly
--     via its own setup-time subtraction).
--   - is_segment_end_mid_shift: the next shift-relative hour is missing or
--     had zero output, AND this is not the last recorded hour of the whole
--     item.
-- Both now widen the existing TEFF trigger (previously only multi-product
-- hours, the item's single first hour, or an explicit "uncontrollable"
-- downtime reason) - no new TEFF formula, per the doc's explicit
-- instruction to reuse the existing one, just fixed to actually engage for
-- every production boundary rather than only the item's global endpoints.
--
-- The item's true LAST recorded hour (is_last_hour, now computed via
-- "no row exists at a later shift-relative position" - equivalent to the
-- old max(hour) but wraparound-safe) is checked FIRST, ahead of the
-- ratio-based TEFF path: per the doc's explicit "ZVLÁŠTNÍ PRAVIDLO", the
-- screenshot_time in the record header is the AUTHORITATIVE end-of-
-- production marker for the shift's final hour and must never be replaced
-- by a derived/ratio-based estimate. auto_shift_productive_minutes() already
-- correctly applies that cutoff when told this is the last hour - it just
-- wasn't reliably being reached before.
...
begin
  ...
  loop
    ...
    if coalesce(r.actual_output, 0) <= 0 and nullif(trim(coalesce(r.product_code, '')), '') is null then
      -- Rule D: a genuinely empty hour (no output, no product - not a
      -- downtime-interrupted production hour, which still has a product)
      -- carries no productive time at all, regardless of its position
      -- relative to neighboring production hours.
      v_minutes := 0;
    ...
    set
      ...
      raw_data = coalesce(raw_data,'{}'::jsonb)
        || jsonb_build_object(
          ...
          || jsonb_build_object(
            'model_version', '2.01-AUTO-reconstruction-v20',
